...
if(existsSync(resolve(root,'docs/qualification/own-rust-cad-v13.json')))throw new Error('Historical own-rust-cad-v13 evidence already exists; publish a new version instead of overwriting it')
const previousPath='docs/qualification/own-rust-cad-v12.json',previous=JSON.parse(bytes(previousPath))
...
writeFileSync(resolve(root,'docs/qualification/own-rust-cad-v13.json'),JSON.stringify({
  id:'own-rust-cad-v13',successorOf:previousPath,previousSha256:sha(bytes(previousPath)),recordedAt:'2026-09-24',
  purpose:'Append-only current-byte fingerprint for the rebuilt geometry kernel after the committed brep-topology EditScope revalidation, raster-core shader variants and PolygonMesh typed-array refactor; the V12 bounded non-manifold and mixed-dimensional topology scope is unchanged.',qualificationClaim:'none',
...
console.log('Recorded docs/qualification/own-rust-cad-v13.json and refreshed src/core/ownRustCadEvidence.ts')
