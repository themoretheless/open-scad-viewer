...
try{
  ...
  for(const [name,viewport] of [['desktop',{width:1280,height:900}],['mobile',{width:390,height:844}]]){
    ...
    await panel.getByLabel('Bed temperature, °C',{exact:true}).fill('80')
    ...
    // Explicit stable axial fixture: all nodes fixed except one loaded top corner.
    // The spatial pin-jointed box with only its base fixed has free mechanisms.
    await panel.getByLabel('Bounding plane',{exact:true}).selectOption('z-max')
    await panel.getByRole('button',{name:'Set restraints',exact:true}).click()
    for(const axis of ['X','Y','Z'])await panel.getByLabel(`Node 4 ${axis} restrained`,{exact:true}).uncheck()
    ...
    await results.waitFor().catch(async error=>{console.error(await panel.innerText());throw error})
    const screening=results.getByRole('table',{name:'Axial demand screening',exact:true})
    assert.equal(await screening.count(),0)
    await results.getByLabel('Tensile limit, MPa',{exact:true}).fill('1')
    await results.getByLabel('Compressive limit, MPa',{exact:true}).fill('1')
    await screening.waitFor()
    assert.equal(await screening.locator('tbody tr').count(),13)
    assert.match(await screening.textContent(),/Overloaded/)
    await results.getByLabel('Safety factor',{exact:true}).fill('0.5')
    await results.getByRole('alert').waitFor()
    assert.equal(await screening.count(),0)
    await results.getByLabel('Safety factor',{exact:true}).fill('2')
    await screening.waitFor()
    ...
    assert.deepEqual(first.printProfile.profile,{material:'PETG',grade:'Test spool',propertySource:'Synthetic regression fixture; not a material certificate',nozzleMm:0.4,lineWidthMm:0.45,layerHeightMm:0.2,nozzleTempC:240,bedTempC:80})
    assert.equal(first.axialScreening.rows.length,13)
    assert.equal(first.axialScreening.limits.safetyFactor,2)
    assert.equal(first.model.nodesMm.length,8)
    ...
    assert.equal(markers.reduce((sum,mesh)=>sum+mesh.triangles,0),13*8)
    ...
    await panel.getByLabel('Nozzle temperature, °C',{exact:true}).fill('245')
    assert.equal(await results.count(),0)
    assert.equal(await panel.getByLabel('Young modulus, MPa',{exact:true}).inputValue(),'')
    assert.equal(await page.evaluate(()=>window.__trussProbe.preview),null)
    await panel.getByLabel('Young modulus, MPa',{exact:true}).fill('4000')
    await panel.getByRole('button',{name:'Solve model',exact:true}).click()
    await results.waitFor()
    assert.equal(await results.getByLabel('Tensile limit, MPa',{exact:true}).inputValue(),'')
    const changedProfile=await downloadReport('changed-profile')
    assert.equal(changedProfile.printProfile.profile.nozzleTempC,245)
    assert.equal(changedProfile.axialScreening,null)
    await panel.getByLabel('Properties from temperature table',{exact:true}).check()
    await panel.getByLabel('Part service temperature, °C',{exact:true}).fill('20')
    const thermalSamples=[{nozzleTempC:245,serviceTempC:20,youngMpa:4000,tensionMpa:40,compressionMpa:60},
      {nozzleTempC:245,serviceTempC:60,youngMpa:2000,tensionMpa:20,compressionMpa:30}]
    for(const [i,sample] of thermalSamples.entries())for(const [key,value] of Object.entries(sample)){
      await panel.getByLabel(`Sample ${i+1} ${key}`,{exact:true}).fill(String(value))
    }
    await panel.getByRole('button',{name:'Bind measurements to current profile',exact:true}).click()
    assert.equal(await panel.getByLabel('Young modulus, MPa',{exact:true}).inputValue(),'4000')
    await panel.getByRole('button',{name:'Solve model',exact:true}).click()
    await results.waitFor()
    const thermalCold=await downloadReport('thermal-cold')
    assert.equal(thermalCold.thermal.result.modelKind,'measured-bilinear-isotropic-axial-v1')
    assert.deepEqual(thermalCold.thermal.request.samples,thermalSamples)
    assert.equal(thermalCold.axialScreening.limits.tensionMpa,40)
    assert.ok(Math.abs(thermalCold.result.maxDeflectionMm-changedProfile.result.maxDeflectionMm)<1e-10)
    await panel.getByLabel('Part service temperature, °C',{exact:true}).fill('60')
    assert.equal(await results.count(),0)
    await panel.getByRole('button',{name:'Solve model',exact:true}).click()
    await results.waitFor()
    const thermalHot=await downloadReport('thermal-hot')
    assert.equal(thermalHot.model.members[0].youngMpa,2000)
    assert.ok(Math.abs(thermalHot.result.maxDeflectionMm-2*thermalCold.result.maxDeflectionMm)<1e-10)
    assert.ok(Math.abs(thermalHot.axialScreening.rows[0].utilization-2*thermalCold.axialScreening.rows[0].utilization)<1e-10)
    await panel.getByLabel('Part service temperature, °C',{exact:true}).scrollIntoViewIfNeeded()
    await page.screenshot({path:`/private/tmp/osv-thermal-strength-${name}.png`})
    await panel.getByLabel('Part service temperature, °C',{exact:true}).fill('61')
    assert.equal(await results.count(),0)
    await panel.getByRole('button',{name:'Solve model',exact:true}).click()
    await panel.getByRole('alert').filter({hasText:'outside the measured range'}).first().waitFor()
    assert.equal(await results.count(),0)
    await panel.getByLabel('Part service temperature, °C',{exact:true}).fill('40')
    await panel.getByLabel('Grade / manufacturer / composition',{exact:true}).fill('Different spool')
    await panel.getByRole('alert').filter({hasText:'does not match'}).first().waitFor()
    assert.equal(await panel.getByLabel('Young modulus, MPa',{exact:true}).inputValue(),'')
    await panel.getByLabel('Grade / manufacturer / composition',{exact:true}).fill('Test spool')
    await page.evaluate(()=>{window.__trussProbe.hold=true})
    await panel.getByRole('button',{name:'Solve model',exact:true}).click()
    ...
    await panel.getByRole('button',{name:'Solve model',exact:true}).click()
    await results.waitFor()
    assert.equal((await downloadReport('thermal-retry')).thermal.request.serviceTempC,20)
    await page.evaluate(()=>{
    ...
    reports.push({name,viewport,bounds,screenshot,controlsScreenshot,nominalNodes:8,nominalMembers:13,
