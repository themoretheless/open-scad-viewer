...
try {
  ...
  const own3mf = await call('tools/call', { name: 'modelgraph_nurbs_export', arguments: { document: ownDocument, format: '3mf' } })
  if (own3mf.isError || Buffer.from(own3mf.content.find(item => item.type === 'resource').resource.blob, 'base64').readUInt32LE(0) !== 0x04034b50) throw new Error('Own NURBS 3MF failed')
  console.log('PASS: own NURBS kernel, three PNG images, STL and 3MF through generated stdio plugin.')
