...
if(existsSync(resolve(root,'docs/qualification/own-rust-cad-v10.json')))throw new Error('Historical own-rust-cad-v10 evidence already exists; publish a new version instead of overwriting it')
const previousPath='docs/qualification/own-rust-cad-v9.json',previous=JSON.parse(bytes(previousPath))
...
writeFileSync(resolve(root,'docs/qualification/own-rust-cad-v10.json'),JSON.stringify({
  id:'own-rust-cad-v10',successorOf:previousPath,previousSha256:sha(bytes(previousPath)),recordedAt:'2026-09-24',
  purpose:'Append-only current-byte fingerprint for the rebuilt geometry kernel and re-pinned npm/Rust dependency files after the committed optimization series; the V12 bounded non-manifold and mixed-dimensional topology scope is unchanged.',qualificationClaim:'none',
...
console.log('Recorded docs/qualification/own-rust-cad-v10.json and refreshed src/core/ownRustCadEvidence.ts')
