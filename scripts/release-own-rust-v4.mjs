...
const previousPath='docs/qualification/own-rust-cad-v3.json'
const previous=JSON.parse(bytes(previousPath))
const paths=[...new Set([
 ...previous.sourceBundle.paths,
 'crates/brep-core/src/analytic_features.rs',
...
const artifact={
 ...
 purpose:'Append-only current-byte fingerprint for the B-rep V7 exact finite blend successors.',
 ...
 claimBoundary:[
  ...
  'It does not approve G0, G1, variable-radius fillets, valence-3 rolling-ball corners, or capability scope beyond the V7 release artifacts.',
 ],
}
writeFileSync(resolve(root,'docs/qualification/own-rust-cad-v4.json'),`${JSON.stringify(artifact,null,2)}\n`)
