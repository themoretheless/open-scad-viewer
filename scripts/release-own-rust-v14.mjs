...
if(existsSync(resolve(root,'docs/qualification/own-rust-cad-v14.json')))throw new Error('Historical own-rust-cad-v14 evidence already exists; publish a new version instead of overwriting it')
const previousPath='docs/qualification/own-rust-cad-v13.json',previous=JSON.parse(bytes(previousPath))
...
writeFileSync(resolve(root,'docs/qualification/own-rust-cad-v14.json'),JSON.stringify({
  id:'own-rust-cad-v14',successorOf:previousPath,previousSha256:sha(bytes(previousPath)),recordedAt:'2026-09-24',
  purpose:'Append-only current-byte fingerprint for the rebuilt geometry kernel after the committed photogrammetry-core pipeline/host_matching and geometry-bridge cad_lattice perf passes; the V12 bounded non-manifold and mixed-dimensional topology scope is unchanged.',qualificationClaim:'none',
...
console.log('Recorded docs/qualification/own-rust-cad-v14.json and refreshed src/core/ownRustCadEvidence.ts')
