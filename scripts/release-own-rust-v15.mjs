...
if(existsSync(resolve(root,'docs/qualification/own-rust-cad-v15.json')))throw new Error('Historical own-rust-cad-v15 evidence already exists; publish a new version instead of overwriting it')
const previousPath='docs/qualification/own-rust-cad-v14.json',previous=JSON.parse(bytes(previousPath))
...
writeFileSync(resolve(root,'docs/qualification/own-rust-cad-v15.json'),JSON.stringify({
  id:'own-rust-cad-v15',successorOf:previousPath,previousSha256:sha(bytes(previousPath)),recordedAt:'2026-09-24',
  purpose:'Append-only current-byte fingerprint for the rebuilt geometry kernel after the committed brep-core analytic surface intersections, gcode-core 3mf packaging/parser and stable expression eval; the V12 bounded non-manifold and mixed-dimensional topology scope is unchanged.',qualificationClaim:'none',
...
console.log('Recorded docs/qualification/own-rust-cad-v15.json and refreshed src/core/ownRustCadEvidence.ts')
