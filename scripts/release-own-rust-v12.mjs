...
if(existsSync(resolve(root,'docs/qualification/own-rust-cad-v12.json')))throw new Error('Historical own-rust-cad-v12 evidence already exists; publish a new version instead of overwriting it')
const previousPath='docs/qualification/own-rust-cad-v11.json',previous=JSON.parse(bytes(previousPath))
...
writeFileSync(resolve(root,'docs/qualification/own-rust-cad-v12.json'),JSON.stringify({
  id:'own-rust-cad-v12',successorOf:previousPath,previousSha256:sha(bytes(previousPath)),recordedAt:'2026-09-24',
  purpose:'Append-only current-byte fingerprint for the rebuilt geometry kernel after the committed perf series (gcode fixed7 emit, subdivision validation cascade, scanline hatch, brep/nurbs FxHash passes) plus in-flight math-core WGSL work present in the tree; the V12 bounded non-manifold and mixed-dimensional topology scope is unchanged.',qualificationClaim:'none',
...
console.log('Recorded docs/qualification/own-rust-cad-v12.json and refreshed src/core/ownRustCadEvidence.ts')
