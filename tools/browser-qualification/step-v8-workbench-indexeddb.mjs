...
const port=4180,url=`http://127.0.0.1:${port}`
...
try{
  ...
  await retained.setInputFiles(resolve('tests/fixtures/step-v8/self-authored-ap242-sphere.step'))
  await page.locator('.cad-workbench').getByText(/AP242 semantics: not-checked/).waitFor({timeout:120000})
  const editor=page.locator('textarea').first()
  await editor.fill((await editor.inputValue())+'\n// STEP /8 lifecycle edit')
  ...
  if(!beforeReload.source.includes('STEP /8 lifecycle edit'))throw Error('edited project was not saved to IndexedDB')
  await page.reload({waitUntil:'domcontentloaded',timeout:120000})
  if(!(await editor.inputValue()).includes('STEP /8 lifecycle edit'))throw Error('IndexedDB project did not reload')
  ...
  await page.locator('.cad-workbench').getByText(/step-interchange\/8; identity preserved/).waitFor({timeout:30000})
  ...
  const result={capability:'step-interchange/8',indexedDbRevision:beforeReload.revision,
    sourceBytes:new TextEncoder().encode(beforeReload.source).byteLength,exportBytes:Buffer.byteLength(exported),
    semanticConformance:'not-checked',regularityCarrier:'sphere',reimported:true,
    userAgent:await page.evaluate(()=>navigator.userAgent)}
  await mkdir('output/qualification/step-interchange-8-v1',{recursive:true})
  await writeFile('output/qualification/step-interchange-8-v1/browser-workbench-indexeddb.json',JSON.stringify(result,null,2)+'\n')
