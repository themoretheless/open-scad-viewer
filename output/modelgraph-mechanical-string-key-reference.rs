...
use super::emit::{json_string, number};
...
pub fn extrude(loops: &[Vec<Point>], thickness: f64) -> String {
    let mut offset = 0;
    let paths: Vec<Vec<usize>> = loops
        .iter()
        .map(|l| {
            let ids = (offset..offset + l.len()).collect();
            offset += l.len();
            ids
        })
        .collect();
    let points: Vec<Point> = loops.iter().flatten().copied().collect();
    format!(
        "linear_extrude(height={})polygon(points={},paths={});",
        number(thickness),
        json_string(&json!(points)),
        json_string(&json!(paths))
    )
}
fn gear_profile(o: &Value, path: &str) -> Result<(Vec<Vec<Point>>, Value)> {
    ...
    if !internal && teeth < minimum {
        return Err(err(path,format!("Gear requires at least {} teeth at this pressure angle; undercut and profile shift are not implemented.",number(minimum))));
    }
...
fn fixed11_key(value: f64) -> u64 {
    debug_assert!(value >= 0.0 && value <= 64.0);
...
impl ThreadMesh<'_> {
    fn point(&mut self, mut t: f64, mut y: f64, outer: bool) -> usize {
        ...
        let key = format!("{}:{t:.11}:{y:.11}", if outer { 'o' } else { 'i' });
...
pub fn thread(o: &Value, path: &str) -> Result<Generated> {
    ...
    let source = format!(
        ...
        json_string(&json!(mesh.points)),
        json_string(&json!(mesh.faces))
    );
