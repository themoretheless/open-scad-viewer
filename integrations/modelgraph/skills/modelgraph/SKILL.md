...
When available, use modelgraph_export with the document for an STL/OBJ. Otherwise use the generated source with openscad_export. Inspect available tool schemas before calling them; read the returned artifact resource. This integration uses an ephemeral catalog per process: save desired documents and export artifacts before the session ends. Do not promise persistence in the MCP catalog.
