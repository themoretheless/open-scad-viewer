...
impl Session {
    fn new(device: &wgpu::Device, shapes: &[Shape], enabled: bool) -> Self {
        ...
        let mut doc =
            curvex::document::Document::new("persistent Metal scene".into(), 2400., 2400.);
        for shape in shapes {
...
fn frame(
    ...
    for shape in shapes {
        draw_shape_cached(
            &painter,
            shape,
            &CanvasTransform::new(Pos2::ZERO, offset, zoom),
            Some(doc.as_ref()),
        );
    }
...
fn main() {
    block_on(async {
        ...
        let (mut device, mut queue) = adapter
            ...
            .await
            .unwrap();
        let shapes = vec![cases()[4].clone()];
        let id = shapes[0].id.clone();
        ...
        let saved = baseline.doc.as_ref().clone();
        ...
        for phase in [
            ...
            match phase {
                "pan" => offset = egui::Vec2::new(12.371, -7.913),
                "fractional-zoom" => {
                    zoom = 7.371;
                    offset = egui::Vec2::new(12.371, -7.913);
                }
                "geometry-edit" => std::rc::Rc::make_mut(&mut baseline.doc)
                    .get_shape_by_id_mut(&id)
                    .unwrap()
                    .translate(7., -4.),
                "recolor" => {
                    if let Some(FillStyle::Gradient { gradient }) =
                        &mut std::rc::Rc::make_mut(&mut baseline.doc)
                            .get_shape_by_id_mut(&id)
                            .unwrap()
                            .fill
                    {
                        gradient.stops[0].color = Color::new(255, 0, 0, 255);
                    }
                }
                "undo-restore" => std::rc::Rc::make_mut(&mut baseline.doc).restore_from(&saved),
                "renderer-reinstall" => {
                    retained.renderer = egui_wgpu::Renderer::new(
                        &device,
                        wgpu::TextureFormat::Rgba8Unorm,
                        Default::default(),
                    );
                    let atlas = retained.ctx.fonts(|fonts| fonts.image());
                    retained.renderer.update_texture(
                        &device,
                        &queue,
                ...
                "device-recreate" => {
                    ...
                    device = pair.0;
                    queue = pair.1;
                    for session in [&mut baseline, &mut retained] {
                        session.renderer = egui_wgpu::Renderer::new(
                            &device,
                            wgpu::TextureFormat::Rgba8Unorm,
                            Default::default(),
                        );
                        let atlas = session.ctx.fonts(|fonts| fonts.image());
                        session.renderer.update_texture(
                            &device,
                            &queue,
                            egui::TextureId::default(),
                            &egui::epaint::ImageDelta::full(atlas, egui::TextureOptions::LINEAR),
                ...
                "fallback" => curvex_metal_renderer::egui_integration::disable(&retained.ctx),
                _ => {}
            }
            retained.doc = baseline.doc.clone();
            let shapes = vec![baseline.doc.get_shape_by_id(&id).unwrap().clone()];
            let (before, _, _, _, _) = frame(&mut baseline, &device, &queue, &shapes, zoom, offset);
            let (after, callbacks, _, _, _) =
                frame(&mut retained, &device, &queue, &shapes, zoom, offset);
            ...
            if phase == "geometry-edit" {
                assert!(after != initial, "edit left stale pixels");
                edited = after.clone();
            }
            if phase == "recolor" {
                assert!(after != edited, "recolor left stale pixels");
            }
            let must_restore = [
                "warm",
                "undo-restore",
            ...
            .contains(&phase);
            let restored_difference = after.iter().zip(&initial).filter(|(a, b)| a != b).count();
            if phase == "fallback" {
                assert_eq!(callbacks, 0);
            } else {
                assert!(callbacks > 0);
            }
            report.push(json!({"phase":phase,"must_restore":must_restore,"restored_changed_channels":restored_difference,"callbacks":callbacks,"max_channel_difference":difference,"coverage_disagreement_pixels":coverage}));
        }
        println!(
            "{}",
            json!({"cases":report,"gate":"same alpha support and at most 1/255 channel difference for fractional camera arithmetic; restored images exact"})
        );
        assert!(
            report
                .iter()
                .all(|r| r["max_channel_difference"].as_u64().unwrap() <= 1
                    && r["coverage_disagreement_pixels"] == 0
                    && (!r["must_restore"].as_bool().unwrap()
                        || r["restored_changed_channels"] == 0)),
            "multi-frame parity failed"
        );
    });
}
