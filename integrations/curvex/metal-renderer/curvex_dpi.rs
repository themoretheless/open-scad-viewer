...
fn capture(
    ...
    let logical_size = 110. * zoom;
    let size = (logical_size * 2.) as u32;
    ...
    ctx.begin_pass(RawInput {
        screen_rect: Some(Rect::from_min_max(
            Pos2::ZERO,
            Pos2::new(logical_size, logical_size),
        )),
        ..Default::default()
    });
    let painter = ctx
        ...
        .with_clip_rect(Rect::from_min_max(
            Pos2::new(3., 5.),
            Pos2::new(logical_size - 7., logical_size - 9.),
        ));
    draw_shape_cached(
        &painter,
        shape,
...
fn main() {
    block_on(async {
        ...
        println!(
            "{}",
            json!({"adapter":format!("{:?}",adapter.get_info()),"cases":report,"pixels_per_point":2,"total_callbacks":total_callbacks})
        );
