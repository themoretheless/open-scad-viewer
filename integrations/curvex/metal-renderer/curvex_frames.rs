        ...
        for count in [1, 100, 1000] {
            ...
            for cycle in 0..12 {
                for slot in if cycle % 2 == 0 {
                    ...
                    let (_, callbacks, c, g, complete) = frame(
                        session,
                        &device,
                        &queue,
                        &shapes,
                    ...
                    raw.push(json!({"uploaded_bytes_total":stats.map(|s|s.0.uploaded_bytes).unwrap_or(0),"retained_bytes":stats.map(|s|s.1).unwrap_or(0),"cycle":cycle,"retained":slot==1,"callbacks":callbacks,"cpu_ms":c,"gpu_ms":g,"completed_draw_ms":complete}));
                }
            }
            report.push(json!({"shapes":count,"completed_baseline_ms":median(&mut completed[0]),"completed_retained_ms":median(&mut completed[1]),"valid_gpu_samples":[gpu[0].len(),gpu[1].len()],"cpu_baseline_ms":median(&mut cpu[0]),"cpu_retained_ms":median(&mut cpu[1]),"gpu_baseline_ms":median(&mut gpu[0]),"gpu_retained_ms":median(&mut gpu[1]),"samples":raw}));
        }
        println!(
            "{}",
            json!({"adapter":format!("{:?}",adapter.get_info()),"workload":"overlapping copies of actual Curvex fixtures; diagnostic, not final document benchmark","cases":report})
        );
    });
}
