...
| 8b. Lifted UV trimming and material classification | `crates/brep-core/src/{trim_sew,uv_arrangement}.rs`, planar_trim | Partial+. Chart events, classify certificates, exact sew, and `uv_arrangement` imprint arrange + missed-branch mutation on frozen charts. General lifted UV DCEL and pole/periodic rules remain open. |
...
| 9c. Regularized curved Boolean / SolidSet | `crates/brep-core/src/{operations,analytic_boolean,uv_arrangement,analytic_ss}.rs` | **Qualified** under `analytic-boolean-1-evidence-v1`: cylinder imprint BooleanCertificate, UV classify+missed-branch, SolidSet body_count, listed analytic SS contacts. Out-of-matrix sphere/torus Boolean refuse. No mesh/Manifold fallback. |
| 10. Canonicalization, exact sewing, explicit healing | `trim_sew::exact_sew`, shared constructor edges | Partial. Exact-match sew with typed gap/duplicate/orientation refusal and rollback drills is implemented; silent healing remains forbidden. Full displacement/error ledger and idempotent production sewing remain open. |
...
| 12. Native/WASM execution, cancellation, ABI, scene v6 | `crates/geometry-wasm`, `src/services/geometry/{kernel,kernelLeases}.ts` | Partial+. Shared WASM plus explicit lease/epoch/cancel/LKG-stale drills (`kernelLeases`) exist beside the diagnostic lane. Full OOM/chunk/scene-v6 production parity remains open. |
| 13. Browser/MCP B-rep activation | `src/mcp/createServer.ts` (`openscad_brep_capabilities`), `brepCapability.ts`, semantic backend leases | Partial+. Honest maturity matrix is MCP-visible; cross-engine fallback remains literal false. Fake “available” for Unavailable/ResearchOnly capabilities is refused. Full same-program production quotas/watchdog remain open. |
| 14. Full qualification and release | `docs/qualification/*-evidence-v1.json`, `brep-capability-registry-release-v1.json`, `brepCapability.ts` | Partial+. Per-capability evidence artifacts + registry release ledger for the Full coverage walking slices; SBOM fingerprints under `docs/qualification/sbom/`. Broader toolchain hash refresh and full schema-strict QualificationPlans remain ongoing. |
| N1. Persistent naming across rebuilds | `TopologyIds`, `src/services/geometry/selectionTransfer.ts`, `topologyLineage.ts` | Partial+. Selection-transfer API maps persist/split/merge/lost/ambiguous without nearest-face guesses. Full roles/anchors and parameter-rebuild correspondence remain open. |
| R1/G6. General NURBS surface intersection | `docs/design/r1-nurbs-ss-spike-adr.md`, `nurbs_ss_g6.rs` | **AnalyticComplete** narrow expand: Complete-empty + planar transverse Complete line; coincident/near-tangent refuse; non-affine `surface_surface` forwards empty/Incomplete. Boolean imprint forbidden. |
| Later features: analytic fillet/chamfer, shell/offset, loft/sweep, direct face edits | `analytic_features.rs` | **AnalyticComplete** walking slices AF-01/AS-01/ASL-01 with FeatureCertificate; chamfer remains Unavailable. Faceted ops must not be relabeled. |
...
| Exchange and persistence | `analytic_features::{export_step,import_step}` | **AnalyticComplete** ADVANCED_FACE / MANIFOLD_SOLID_BREP; FACETED_BREP/STL/OBJ refuse. Full IGES/general NURBS STEP still open. |
