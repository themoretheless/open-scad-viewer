...
| 5. Numeric context, certified predicates/constructions, typed ambiguity | `crates/cad-predicates`, `predicate_evidence.rs`, ADR 0006 | **InProgress** production-candidate: cad-predicates bridged into Complete corroboration (orient3d/orient2d); G2 gate not closed. |
...
| 10. Canonicalization, exact sewing, explicit healing | `trim_sew` DisplacementRecord ledger | **Closed** for exact-match sew + displacement budget refuse; silent heal forbidden. Authorized heal plans remain post-matrix. |
| 11. Certified shell-aware tessellation | `geometry-bridge/src/brep.rs` TessellationCertificate | **Closed** for matrix shells + freeform notes (`freeform_nurbs_tessellation_sampled`); deviation oracles remain out-of-scope. |
...
| 14. Full qualification and release | `brep-closed-matrix-v1.json` + `plans/*.json` + `sbom/brep-closed-matrix-release-v1.json` + registry-full | **Closed** for the frozen shipped matrix under g8-full; immutable matrix hash is pinned, schema-strict plans are green, and unresolved shipped rows are empty. |
...
| R1/G6. General NURBS surface intersection | `docs/design/r1-nurbs-ss-spike-adr.md`, `nurbs_ss_g6.rs` | **AnalyticComplete** Phase B (`nurbs-ss-bezier-le3/1`): Bezier deg≤3 elevated to bicubic Complete-empty/line plus certified non-planar constant-parameter iso-curve; F4 Boolean imprint `nurbs-boolean-bezier-le3/1` (`booleanImprint: true`, topology change permitted). Parasolid still refuse. |
| Later features: analytic fillet/chamfer, shell/offset, loft/sweep, direct face edits | `analytic_features.rs` | **AnalyticComplete** AF-01/AC-01/AS-01/ASL-01; FrameLaw stub fail-closed; mesh bevel quarantined on B-rep bodies. |
...
| Exchange and persistence | `export_step/import_step`, `export_nurbs_step*`, `export_iges/import_iges` | **AnalyticComplete** constructor STEP + freeform face/trim/solid STEP (`nurbs-step-bicubic-face/1`, `nurbs-step-trimmed-bicubic/1`, `nurbs-step-solid/1` / `cadNurbsStep.ts`) + IGES; FACETED/STL/OBJ and arbitrary third-party AP214 entity graphs refuse; Parasolid parity out of scope. **STEP+NURBS product-matrix closed.** |
