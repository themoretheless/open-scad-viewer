...
Продолжение после слияния: [совместный анализ retained solid](solid-analysis-2026-09-19.md)
объединяет display mesh, BVH и рёбра внутри Rust. На одинаковой сцене из
256 тел медиана анализа уменьшилась на 36.2%; на плотной сетке заметного
ускорения нет. Хеширование, surface IDs, отмена внутри ядра и валидация
публикации остаются открытыми частями пункта 3 плана ниже.
