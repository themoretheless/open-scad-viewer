...
`modelgraph/nurbs-1` явно выбирает собственные численные алгоритмы на TypeScript. NURBS-вычисления не используют стороннюю геометрическую библиотеку, Open CASCADE, CadQuery или Manifold. Zod проверяет входной JSON, а `tsx` запускает TypeScript; эти зависимости не вычисляют геометрию. Прежние `modelgraph/1` и OpenSCAD сохраняют свои маршруты. Зарезервированное Rust B-rep-ядро этим изменением не включается.
...
flowchart LR
  ...
  B --> C[Собственные рациональные NURBS]
