...
const EXTENSIONS: Readonly<Record<MeshImportFormat, readonly string[]>> = Object.freeze({
  stl: ['stl'],
  obj: ['obj'],
  ply: ['ply'],
  off: ['off'],
...
const MIME_TYPES: Readonly<Record<MeshImportFormat, readonly string[]>> = Object.freeze({
...
export const MESH_IMPORT_ACCEPT = MESH_IMPORT_FORMATS
  .flatMap(format => [...EXTENSIONS[format].map(extension => `.${extension}`), ...MIME_TYPES[format]])
...
export function detectMeshImportFormat(fileName: string, bytes?: Uint8Array): MeshImportFormat | null {
  ...
  for (const format of MESH_IMPORT_FORMATS) {
    if (EXTENSIONS[format].some(extension => lower.endsWith(`.${extension}`))) return format
  }
...
export function stripMeshExtension(fileName: string): string {
  const lower = fileName.toLowerCase()
  for (const format of MESH_IMPORT_FORMATS) {
    for (const extension of EXTENSIONS[format]) {
