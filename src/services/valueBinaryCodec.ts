...
export function decodeBinary(data:Uint8Array):unknown {
 ...
 const get=(depth:number):unknown=>{
  ...
  if(tag===5||tag===6){const n=count();if(n>data.length-offset||n>4_000_000-nodes)throw new Error('Binary item limit');if(tag===5){const out:unknown[]=[];for(let i=0;i<n;i++)out.push(get(depth+1));return out}const out:Record<string,unknown>={};for(let i=0;i<n;i++){const k=get(depth+1);if(typeof k!=='string'||Object.hasOwn(out,k))throw new Error('Invalid or duplicate binary object key');Object.defineProperty(out,k,{value:get(depth+1),enumerable:true,writable:true,configurable:true})}return out}
