...
type Ast = { kind: string; value?: string; generics?: string[]; inputs?: Field[]; outputs?: Field[]; singleResult?: boolean; types?: TypeRef[]; items?: Ast[]; args?: { name?: string; value: Ast }[]; left?: Ast; right?: Ast }
type RecordValue = { record: Map<string, Value>; recordType?: TypeRef }
...
export function compileModelGraphText(source: string) {
 ...
 function layoutFunction(base:string):Ast {
  ...
  inner();const inputs=peek()==='->'?[]:listFields();inner();take('->');signatureLine()
  const singleResult=tokens[p+1]?.text!==':'
  const outputs=singleResult?[{name:'value',type:typeRef()}]:listFields()
  ...
  while(peek()!=='ret'){
   ...
   if(![':', '='].includes(peek()))fail('Expected : or = for local binding')
   take();items.push({kind:'binding',left:binding,right:expr()})
   if(items.length>64)fail('At most 64 function statements')
   if(peek()!=='\n')fail('Expected newline after local binding')
   inner()
  }
