...
export function bodyFingerprint(body:DirectBody):string{const points=Array.from({length:body.mesh.positions.length/3},(_,i)=>body.mesh.positions.slice(i*3,i*3+3).map(v=>Math.round(v*1e5)).join(',')),triangles=Array.from({length:body.mesh.indices.length/3},(_,i)=>body.mesh.indices.slice(i*3,i*3+3).map(j=>points[j]).sort().join(';')).sort();let h=2166136261;for(const c of triangles.join('|')){h^=c.charCodeAt(0);h=Math.imul(h,16777619)}return (h>>>0).toString(16)}
...
export function jointOptions(source:string,bodies:DirectBody[],o:CadOptions,min:number,max:number):CadOptions{
 ...
 if(old){if(old.parentShape!==bodyFingerprint(bodies[parent])||old.childShape!==bodyFingerprint(bodies[child]))throw Error('Joint geometry changed. Remove the saved joint and create a new reference.');if(old.kind!==o.mode||JSON.stringify(old.axis)!==JSON.stringify(o.axis)||JSON.stringify(old.origin)!==JSON.stringify(o.origin))throw Error('Remove the saved joint before changing its axis, origin or kind.')}
