...
export function highlightCode(source: string, selectedName = ''): string {
  ...
  const keywords = new Set(['foreach', 'yield', 'continue', 'break', 'fn', 'ret', 'struct', 'int', 'f32', 'f64', 'str', 'Geometry', 'in', 'by', 'count', 'where', 'validate', 'param', 'range', 'show', 'module', 'function', 'let', 'each', 'for', 'if', 'else', 'assert', 'echo', 'include', 'use', 'true', 'false', 'undef']);
  let result = '', end = 0;
