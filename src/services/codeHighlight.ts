...
export function highlightCode(source: string): string {
  ...
  const keywords = new Set(['param', 'range', 'show', 'module', 'function', 'let', 'each', 'for', 'if', 'else', 'assert', 'echo', 'include', 'use', 'true', 'false', 'undef']);
  let result = '', end = 0;
  for (const match of source.matchAll(tokens)) {
    ...
    const kind = value.startsWith('//') || value.startsWith('/*') ? 'comment'
      ...
      : 'operator';
    result += kind ? `<span class="syntax-${kind}">${escape(value)}</span>` : escape(value);
