...
export function exportMeshFormat(mesh: NurbsMesh, format: MeshExportFormat) {
    ...
    if (topology.degenerateTriangles || topology.nonManifoldEdges || topology.orientationConflicts)
        throw new Error('Mesh has invalid or inconsistent topology.');
    if (['stl', 'stl_binary', '3mf', 'amf'].includes(format) && (!topology.closed || topology.signedVolumeMm3 <= 0))
    ...
    if (format === 'stl') {
        ...
        const model = '<?xml version="1.0" encoding="UTF-8"?><model unit="millimeter" xml:lang="en-US" xmlns="http://schemas.microsoft.com/3dmanufacturing/core/2015/02"><resources><object id="1" type="model"><mesh><vertices>' + points.map(p => `<vertex x="${p[0]}" y="${p[1]}" z="${p[2]}"/>`).join('') + '</vertices><triangles>' + faces.map(f => `<triangle v1="${f[0]}" v2="${f[1]}" v3="${f[2]}"/>`).join('') + '</triangles></mesh></object></resources><build><item objectid="1"/></build></model>';
