...
export function cadOperation(input:DirectDocument,o:CadOptions):DirectDocument{
 ...
 if(o.action==='mirror'){
  ...
  const mirrored=callGeometryRust<DirectBody[]>('cad_mirror_bodies',{bodies:selected,origin:o.origin,axis:o.axis})
 ...
 if(o.action==='align'||o.action==='distribute'){
  const bodies=callGeometryRust<DirectBody[]>('cad_arrange_bodies',{bodies:selected,axis:o.axis,action:o.action,mode:o.mode})
  bodies.forEach(replace);return d
 }
 if(o.action==='joint'){
  const bodies=callGeometryRust<DirectBody[]>('cad_joint_bodies',{bodies:selected,axis:o.axis,origin:o.origin,amount:o.amount,mode:o.mode})
  bodies.forEach(replace);return d
 }
 if(o.action==='pattern'){
  const groups=callGeometryRust<DirectBody[][]>('cad_pattern_bodies',{bodies:selected,axis:o.axis,amount:o.amount,count:o.count,path:o.sketches.find(s=>s.id===o.pathId)??null})
 ...
 if(['union','difference','intersection'].includes(o.action)){
  const result=callGeometryRust<DirectBody[]>('cad_boolean_bodies',{bodies:selected,operation:o.action})
 ...
 if(o.action==='hole'){
  if(!selected.length)throw Error('Select bodies.')
  replace(callGeometryRust<DirectBody>('cad_hole_body',{body:selected[0],axis:o.axis,origin:o.origin,width:o.width,depth:o.depth,height:o.height,secondary:o.secondary,mode:o.mode}))
  return d
 }
 if(o.action==='draft'){
  callGeometryRust<DirectBody[]>('cad_draft_bodies',{bodies:selected,axis:o.axis,origin:o.origin,amount:o.amount}).forEach(replace)
  return d
 }
 if(o.action==='loft'||o.action==='sweep'){
  const mesh=callGeometryRust<PolygonMesh>('cad_build_sections',{action:o.action,sketches:o.sketches,profileIds:o.profileIds,pathId:o.pathId})
  d.bodies.push({id:crypto.randomUUID(),name:o.action==='loft'?'Loft':'Sweep',mesh});return d
 }
 if(o.action==='thread'){
  if(!selected.length)throw Error('Select bodies.')
  replace(callGeometryRust<DirectBody>('cad_thread_body',{body:selected[0],axis:o.axis,origin:o.origin,width:o.width,depth:o.depth,pitch:o.pitch,mode:o.mode}));return d
 }
 if(!selected.length)throw Error('Select bodies.')
 ...
 if(o.action==='resize'){
  const bodies=callGeometryRust<DirectBody[]>('cad_resize_bodies',{bodies:selected,desired:[o.width,o.height,o.depth]})
  bodies.forEach(replace);return d
 }
