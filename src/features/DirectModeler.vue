...
function booleanPair(a:DirectBody,b:DirectBody,operation:BrepBooleanOperation):{body:DirectBody|null;exact:boolean;toleranceMm?:number}{
 if(a.brep&&b.brep){
  try{
   const brep=booleanNurbsBrep(a.brep,b.brep,operation)
   const result=bodyFromBrep(a,brep)
...
function applyBrepBoolean(operation:BrepBooleanOperation){run(()=>{
 ...
 notice.value=''
 const {body:result,exact,toleranceMm}=booleanPair(bodies[0],bodies[1],operation)
 if(!exact&&result)notice.value=FALLBACK_NOTICE()
 else if(toleranceMm!==undefined&&result)notice.value=TOLERANT_NOTICE(toleranceMm)
...
function applySubtract(){run(()=>{
 ...
 let exact=true,tolerance=0
 const fold=(ids:string[])=>ids.slice(1).reduce<DirectBody>((acc,id)=>{const r=booleanPair(acc,bodyOf(id),'union');exact&&=r.exact;tolerance=Math.max(tolerance,r.toleranceMm??0);if(!r.body)throw Error(EMPTY_NOTICE());return r.body},bodyOf(ids[0]))
 const a=fold(s.a),b=fold(s.b)
 const r=booleanPair(a,b,'difference');exact&&=r.exact;tolerance=Math.max(tolerance,r.toleranceMm??0)
 ...
 if(!r.body)notice.value=EMPTY_NOTICE();else if(!exact)notice.value=FALLBACK_NOTICE();else if(tolerance>0)notice.value=TOLERANT_NOTICE(tolerance)
 subtract.value=null
})}
...
function persist() {
  ...
  if (text.length > MAX_DRAFT_CHARACTERS) {
    ...
    error.value = label('Документ слишком большой для черновика в браузере. Скачайте JSON, чтобы сохранить.', 'Document is too large for the browser draft. Download JSON to keep it.')
    return
  }
...
async function importFile(event: Event) {
  ...
  try { if (file) { if (file.size > MAX_DOCUMENT_CHARACTERS) throw new Error('Document exceeds 64 MB.'); const text = await file.text()
