    ...
    <aside class="side-dock" :class="{ collapsed: !dockOpen }" :aria-label="label('Панель', 'Panel')">
      ...
      <div v-if="dockOpen" class="dock-body">
        <template v-if="dockTab === 'scene'">
          <div class="dock-heading">{{ label('Сцена', 'Scene') }} <span>{{ document.bodies.length + document.sketches.length + (document.curves?.length ?? 0) + (document.surfaces?.length ?? 0) }}</span></div>
          <ul class="scene-list">
            ...
            <li v-if="document.bodies.length" class="scene-group">{{ label('Тела', 'Bodies') }}</li>
            <li v-for="item in document.bodies" :key="item.id"><button type="button" :aria-label="item.name" :aria-pressed="selectedIds.includes(item.id)" @click="pickObject(item.id,'3d',$event.shiftKey)"><span class="dot body"></span>{{ item.name }}<small v-if="item.brep">B-rep</small></button></li>
