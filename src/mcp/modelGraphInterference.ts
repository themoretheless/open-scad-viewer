...
export async function inspectModelGraphInterference(document: unknown, geometry: McpGeometryService, signal?: AbortSignal) {
  ...
  if (leaves.length > 32) throw new Error('Interference checking supports at most 32 leaf components per call.')
  ...
  for (let i=0;i<leaves.length;i++) for (let j=i+1;j<leaves.length;j++) {
    ...
    if (!ab || !bb) { pairs.push({a:a.instance_path,b:b.instance_path,status:'unknown',intersection_volume_mm3:null,method:'bounds_unavailable'});continue }
    if (ab.min.some((v,k)=>v >= bb.max[k]! || ab.max[k]! <= bb.min[k]!)) {
...
export function registerModelGraphInterference(server:McpServer, geometry:McpGeometryService) {
  server.registerTool('modelgraph_interference',{description:'Check volume overlap between up to 32 leaf assembly components at current joint positions using bounds and actual Manifold intersections. Does not check contact, clearance or the swept path of motion.',inputSchema:z.object({document:modelGraphSchema}),annotations:{readOnlyHint:true,openWorldHint:false}},async(input,context)=>{
    ...
    catch {return {isError:true,content:[{type:'text' as const,text:'Interference check unavailable. Use an assembly with at most 32 leaf components, validate it with modelgraph_check and retry.'}]}}
  })
}
