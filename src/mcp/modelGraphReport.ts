...
export function registerModelGraphReport(server: McpServer, geometry: McpGeometryService) {
  server.registerTool('modelgraph_report', {
    ...
    try {
      ...
      const report = { document_sha256: compiled.document_sha256, analysis: built.analysis, constraints: compiled.constraint_report, sketch_solutions: compiled.sketch_solutions, assembly_components: compiled.assembly_components, mechanical_reports: compiled.mechanical_reports, mechanical_parts: compiled.mechanical_parts, references: references.slice(0, 2048), references_truncated: references.length > 2048, reference_scope: 'generated-operation provenance; not persistent face identifiers', images_status: imageError ? 'unavailable' : 'rendered', image_error: imageError, views: previews.map(item => item.view), printability: 'unknown' }
      return { structuredContent: report, content: [{ type: 'text' as const, text: JSON.stringify(report) }, ...previews.map(item => ({ type: 'image' as const, mimeType: 'image/png', data: item.png.toString('base64') }))] }
