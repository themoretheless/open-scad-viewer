...
pub fn simplify_with_progress(
    input: &Surface, tolerance: f64, passes: usize,
    ...
    for (fi,t) in input.triangles.iter().enumerate() {
        ...
        for [a,b] in [[t[0],t[1]],[t[1],t[2]],[t[2],t[0]]] {
            let (a,b)=(a as usize,b as usize);let ra=root(&mut parent,a);let rb=root(&mut parent,b);parent[ra]=rb;
            let e=oriented_edges.entry((a.min(b) as u32,a.max(b) as u32)).or_default();e.0+=1;e.1+=if a<b {1}else{-1};
        }
    }
    let component:Vec<_>=(0..parent.len()).map(|i|root(&mut parent,i)).collect();
    let mut closed=vec![true;parent.len()];
    for (&(a,_),&(count,winding)) in &oriented_edges {if count!=2 || winding!=0 {closed[component[a as usize]]=false;}}
    let mut edge_capacity_hint = oriented_edges.len();
    drop(oriented_edges);drop(parent);
    let contribution=|t:[u32;3], c:usize| {
        let origin=input.positions[c];let p=t.map(|i|sub(input.positions[i as usize],origin));dot(p[0],cross(p[1],p[2]))/6.
    };
    let mut remaining=vec![0.;input.positions.len()];
    for &t in &input.triangles {let c=component[t[0] as usize];remaining[c]+=contribution(t,c);}
    for value in &mut remaining {*value=value.abs()*0.01;}
    ...
    for pass in 0..passes {
        ...
        {
            if fi % 4096 == 0 { super::cancelled(&mut progress, "simplify-index", fi, faces.len())?; }
            for &i in &t {
                let v = i as usize;
            ...
            for [a, b] in [[t[0], t[1]], [t[1], t[2]], [t[2], t[0]]] {
                ...
                if edge.0 < 2 { edge.1[edge.0 as usize] = fi as u32; }
                edge.0 += 1;
            }
        }
        ...
        let normals: Vec<_> = faces.iter().map(|t| t.map(|t| {
            let p=t.map(|i|input.positions[i as usize]);
            cross(sub(p[1],p[0]),sub(p[2],p[0]))
        }).unwrap_or([0.;3])).collect();
        let mut locked = vec![false; input.positions.len()];
        for (&(a, b), f) in &edges {
            let (a,b)=(a as usize,b as usize);
            // Protect sharp creases, including the rim of thin closed shells.
            let crease = f.0==2 && dot(normals[f.1[0] as usize],normals[f.1[1] as usize]) <= 0.5 * norm(normals[f.1[0] as usize]) * norm(normals[f.1[1] as usize]);
            if f.0 != 2 || crease {
                locked[a] = true;
                locked[b] = true;
            }
        ...
        for (ci, (length, a, b)) in candidates.into_iter().enumerate() {
            ...
            if closed[c] {
                let mut delta=0.;
                for &fi in &ring {
                    let old=faces[fi].unwrap();delta-=contribution(old,c);
                    if !(old.contains(&(a as u32))&&old.contains(&(b as u32))) {
                        delta+=contribution(old.map(|i|if i as usize==b {a as u32}else{i}),c);
                    }
                }
                if !delta.is_finite() || delta.abs()>remaining[c] {continue;}
    ...
    for (fi, t) in faces.into_iter().flatten().enumerate() {
        if fi % 4096 == 0 { super::cancelled(&mut progress, "simplify-compact", fi, count)?; }
...
fn cancellation_preserves_source_and_budget_precedes_work() {
    let mesh=Surface { positions:vec![[0.,0.,0.],[1.,0.,0.],[0.,1.,0.]],colors:vec![[5;3];3],triangles:vec![[0,1,2]] };
    ...
    for stop in ["simplify", "simplify-index", "simplify-compact"] {
        ...
        assert_eq!(mesh.positions,original.positions);assert_eq!(mesh.triangles,original.triangles);assert_eq!(mesh.colors,original.colors);
    }
    let oversized_faces=Surface {triangles:vec![[0;3];1_000_001],..Default::default()};
    assert!(matches!(simplify(&oversized_faces,1.,1),Err(ref e) if e=="Simplification input budget exceeded"));
    let oversized=Surface {positions:vec![[0.;3];500001],..Default::default()};
    assert!(matches!(simplify(&oversized,1.,1),Err(ref e) if e=="Simplification input budget exceeded"));
}
...
fn close_opposing_sheets_preserve_identity_orientation_and_area() {
    ...
    for &t in &output.triangles {
        ...
        assert!(t.iter().all(|&i|output.colors[i as usize][0] as usize==layer));
    ...
    for area in areas {assert!((area-1.9*1.9).abs()<1e-10);}
    ...
    for (p,c) in mesh.positions.iter().zip(&mesh.colors) {
        if p[0]==0.||p[1]==0.||p[0]==19.*0.1||p[1]==19.*0.1 {
            assert!(output.positions.iter().zip(&output.colors).any(|(q,d)|p==q&&c==d));
        }
    }
    let mut reached=false;
    let result=simplify_with_progress(&mesh,0.4,16,|stage,_,_| {
        if stage=="simplify-collapse" {reached=true;false} else {true}
    });
    assert!(reached);
    assert!(matches!(result,Err(ref e) if e=="Cancelled"));
}
...
fn thin_closed_shell_retains_volume() {
    let mut mesh=Surface::default();let side=10u32;let count=side*side;
    for z in [0.,0.00001] {for y in 0..side {for x in 0..side {mesh.positions.push([x as f64*0.1,y as f64*0.1,z]);mesh.colors.push([128;3]);}}}
    for y in 0..side-1 {for x in 0..side-1 {
        let i=y*side+x;
    ...
    for x in 0..side {ring.push(x);}
    for y in 1..side {ring.push(y*side+side-1);}
    for x in (0..side-1).rev() {ring.push((side-1)*side+x);}
    for y in (1..side-1).rev() {ring.push(y*side);}
    for j in 0..ring.len() {let a=ring[j];let b=ring[(j+1)%ring.len()];mesh.triangles.extend([[a,b,b+count],[a,b+count,a+count]]);}
    let volume=|m:&Surface|m.triangles.iter().map(|t|{let p=t.map(|i|m.positions[i as usize]);dot(p[0],cross(p[1],p[2]))/6.}).sum::<f64>().abs();
    let original=volume(&mesh);assert!((original-0.81*0.00001).abs()<1e-12);
    let out=simplify(&mesh,0.4,16).unwrap();
    assert!(out.triangles.len()<mesh.triangles.len());
    assert!(volume(&out)>=original*0.99,"volume ratio {}",volume(&out)/original);
}

#[cfg(test)]
#[test]
fn smooth_thin_torus_volume_diagnostic() {
    let mut mesh=Surface::default();let (nu,nv)=(64u32,16u32);
    for u in 0..nu {for v in 0..nv {
        let a=u as f64*std::f64::consts::TAU/nu as f64;let b=v as f64*std::f64::consts::TAU/nv as f64;
        let r=1.+0.02*b.cos();mesh.positions.push([r*a.cos(),r*a.sin(),0.02*b.sin()]);mesh.colors.push([128;3]);
    }}
    for u in 0..nu {for v in 0..nv {
        let a=u*nv+v;let b=((u+1)%nu)*nv+v;let c=u*nv+(v+1)%nv;let d=((u+1)%nu)*nv+(v+1)%nv;
    ...
    for tolerance in [0., 0.002, 0.005, 0.01, 0.015] {
        ...
        println!("TORUS_TOLERANCE={tolerance} VOLUME_RATIO={ratio} FACES_BEFORE={} FACES_AFTER={}",mesh.triangles.len(),out.triangles.len());
        assert!(ratio.is_finite());
        assert!(ratio>=0.99-1e-10 && ratio<=1.01+1e-10);
        if tolerance==0. {assert_eq!(out.triangles.len(),mesh.triangles.len());assert_eq!(ratio,1.);}
    }
    // Separate opposite-winding components must not cancel each other's budgets.
    for color in &mut mesh.colors {*color=[0;3];}
    let offset=mesh.positions.len() as u32;
    let old_positions=mesh.positions.clone();let old_faces=mesh.triangles.clone();
    for p in old_positions {mesh.positions.push([p[0]+10.,p[1],p[2]]);mesh.colors.push([1,0,0]);}
    for t in old_faces {mesh.triangles.push([t[0]+offset,t[2]+offset,t[1]+offset]);}
    let output=simplify(&mesh,0.015,16).unwrap();
    let component_volume=|m:&Surface,layer:u8| {
        m.triangles.iter().filter(|t|m.colors[t[0] as usize][0]==layer).map(|t| {
            let p=t.map(|i|sub(m.positions[i as usize],[10.*layer as f64,0.,0.]));dot(p[0],cross(p[1],p[2]))/6.
        }).sum::<f64>().abs()
    };
    for layer in [0,1] {let ratio=component_volume(&output,layer)/component_volume(&mesh,layer);assert!(ratio>=0.99-1e-10&&ratio<=1.01+1e-10);}
