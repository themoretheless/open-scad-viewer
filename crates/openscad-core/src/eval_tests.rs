...
use crate::eval::{Evaluation, EvaluatorOptions, MAX_EVAL_OPS, evaluate_source};
use crate::value::{
    Value, compare_values, deep_equal, format_number, format_value, js_number_to_string,
    js_to_precision, truthy,
};
...
fn eval_ok(source: &str, profile: LanguageProfile) -> Evaluation {
    match evaluate_source(source, profile, EvaluatorOptions::default()) {
        ...
        Err(diagnostic) => panic!(
            ...
            diagnostic.message, diagnostic.start
        ),
    }
}
...
fn truthiness_follows_openscad() {
    ...
    assert!(truthy(&Value::<'static>::Range {
        start: 1.0,
        step: 1.0,
        end: 0.0
    }));
...
fn js_number_formatting_matches_ecmascript() {
    ...
    assert_eq!(
        js_number_to_string(std::f64::consts::PI),
        "3.141592653589793"
    );
...
fn subset_variables_and_expressions() {
    ...
    assert_eq!(
        eval_err("cube(1/0);", SUBSET),
        "Expression produced a non-finite number"
    );
    assert_eq!(
        eval_err("cube([0:0:1]);", SUBSET),
        "Range step cannot be zero"
    );
    assert_eq!(
        eval_err("cube(1 < \"a\");", SUBSET),
        "comparison operand must be a finite number"
    );
}
...
fn subset_control_flow_and_loops() {
    assert_eq!(
        shape_count("for (i = [0:2]) translate([i * 2, 0, 0]) cube(1);", SUBSET),
        3
    );
    assert_eq!(
        shape_count("assert(true) if (true) let(x = 2) cube(x);", SUBSET),
        1
    );
    ...
    assert_eq!(
        eval_err("for (i = 5) cube(1);", SUBSET),
        "for() iterator must be a vector or range"
    );
}
...
fn subset_statements_and_modules() {
    ...
    assert_eq!(
        shape_count(
            "module pair() { cube(1); translate([2,0,0]) cube(1); } pair();",
            SUBSET
        ),
        2
    );
    assert_eq!(
        shape_count(
            "module pick(i) { children(i); } pick(1) { cube(1); cube(2); }",
            SUBSET
        ),
        1
    );
    assert_eq!(
        shape_count(
            "module pick(i) { children(i); } pick(9) { cube(1); cube(2); }",
            SUBSET
        ),
        0
    );
    ...
    assert_eq!(
        shape_count("module m(a, b = a + 1) { cube([a, b, 1]); } m(1);", SUBSET),
        1
    );
}
...
fn subset_geometry_argument_validation_is_kept() {
    assert_eq!(
        eval_err("cube(0);", SUBSET),
        "Cube dimensions must be positive"
    );
    assert_eq!(
        eval_err("sphere(0);", SUBSET),
        "Sphere radius must be positive"
    );
    assert_eq!(
        eval_err("cylinder(0, 1);", SUBSET),
        "Cylinder height must be positive"
    );
    assert_eq!(
        eval_err("cylinder(1, 0, 0);", SUBSET),
        "Cylinder radii must be non-negative and not both zero"
    );
    assert_eq!(
        eval_err("square(-1);", SUBSET),
        "Square dimensions must be positive"
    );
    assert_eq!(
        eval_err("circle(d = 0);", SUBSET),
        "Circle radius must be positive"
    );
    assert_eq!(
        eval_err("linear_extrude(-1) square(1);", SUBSET),
        "linear_extrude() height must be positive"
    );
    assert_eq!(
        eval_err("linear_extrude(1) cube(1);", SUBSET),
        "linear_extrude() requires 2D children"
    );
    assert_eq!(
        eval_err("offset(1) cube(1);", SUBSET),
        "offset() requires 2D children"
    );
    assert_eq!(
        eval_err("projection() square(1);", SUBSET),
        "projection() requires 3D children"
    );
    assert_eq!(
        eval_err("union() { cube(1); square(1); }", SUBSET),
        "union() cannot mix 2D and 3D children"
    );
    assert_eq!(
        eval_err("scale(0) cube(1);", SUBSET),
        "Scale values cannot be zero"
    );
    assert_eq!(
        eval_err("rotate(45, [0, 0, 0]) cube(1);", SUBSET),
        "Rotation axis cannot be zero"
    );
    assert_eq!(
        eval_err("color(\"no-such\") cube(1);", SUBSET),
        "Unknown color no-such"
    );
    assert_eq!(
        eval_err("foo();", SUBSET),
        "Unsupported geometry operation foo()"
    );
    assert_eq!(
        eval_err("minkowski() { cube(1); }", SUBSET),
        "Unsupported geometry operation minkowski()"
    );
    assert_eq!(
        eval_err("echo(\"hi\");", SUBSET),
        "Unsupported geometry operation echo()"
    );
}
...
fn subset_boolean_and_transform_shape_algebra() {
    ...
    assert_eq!(
        shape_count("hull() { cube(1); translate([2,0,0]) cube(1); }", SUBSET),
        1
    );
    assert_eq!(
        shape_count(
            "linear_extrude(height = 4) difference() { square([2, 3]); translate([0.5, 0.5]) square([1, 1]); }",
            SUBSET
        ),
        1
    );
    assert_eq!(
        shape_count(
            "rotate_extrude($fn = 16) translate([2, 0]) square([1, 1]);",
            SUBSET
        ),
        1
    );
    assert_eq!(
        shape_count(
            "linear_extrude(height = 1) offset(r = 0.1) projection() cube(1);",
            SUBSET
        ),
        1
    );
    assert_eq!(
        shape_count("translate([1,2,3]) { cube(1); cube(1); }", SUBSET),
        2
    );
    ...
    assert!(
        two_d
            .warnings
            .iter()
            .any(|w| w.contains("top-level 2D object"))
    );
}
...
fn subset_assert_statement_semantics() {
    ...
    assert_eq!(
        eval_err("assert(false) cube(1);", SUBSET),
    ...
    assert_eq!(
        eval_err("x = let(a = 1) a; cube(1);", SUBSET),
        "Expected ; after assignment"
    );
    assert_eq!(
        eval_err("x = echo(1) 2; cube(1);", SUBSET),
        "Expected ; after assignment"
    );
    assert_eq!(
        eval_err("x = [for (i = [0:2]) i]; cube(1);", SUBSET),
        "Expected ]"
    );
}
...
fn subset_builtins_and_errors() {
    ...
    assert_eq!(
        eval_err("cube(abs(\"a\"));", SUBSET),
        "abs() argument 1 must be a number"
    );
    assert_eq!(
        eval_err("cube(abs(1, 2));", SUBSET),
    ...
    assert_eq!(
        eval_err("cube(nosuchfn(1));", SUBSET),
        "Unsupported function nosuchfn()"
    );
    assert_eq!(
    ...
    assert_eq!(
        shape_count("cube(str(1, \"x\") == \"1x\" ? 1 : 2);", SUBSET),
        1
    );
}
...
fn subset_budget_limits() {
    big_stack(|| {
        ...
        assert!(
            message
                == format!(
                    ...
                    crate::value::locale(MAX_EVAL_OPS as usize)
                )
        ...
        // Ranges are capped.
        assert_eq!(
            eval_err("cube([0:100000]);", SUBSET),
            "Range exceeds 10,000 items"
        );
        ...
        assert!(
            message.contains("value-allocation budget") || message.contains("exceeds"),
            "{message}"
        );
    });
}
...
fn stable_lazy_scope_semantics() {
    ...
    assert!(
        evaluation
            .warnings
            .contains(&"Ignoring unknown variable 'unknown_name'".to_string())
    );
...
fn stable_functions_and_recursion() {
    ...
    assert_eq!(
        shape_count(
            "function fib(n) = n <= 1 ? n : fib(n - 1) + fib(n - 2); cube(fib(10) == 55 ? 1 : 2);",
            STABLE
        ),
        1
    );
    ...
    assert_eq!(
        shape_count(
            "a = 2; f = function(x) x + a; cube(f(1) == 3 ? 1 : 2);",
            STABLE
        ),
        1
    );
    ...
    assert_eq!(
        shape_count(
            "function g(a, b = 10) = a + b; cube(g(b = 1, a = 2) == 3 ? 1 : 0);",
            STABLE
        ),
        1
    );
    ...
    assert!(
        evaluation
            .warnings
            .contains(&"Ignoring excess positional argument".to_string())
    );
    let evaluation = eval_ok("function h(a) = a; cube(h(1, bogus = 2));", STABLE);
    assert!(
        evaluation
            .warnings
            .contains(&"Ignoring unknown argument bogus".to_string())
    );
    // Functions shadowed by variables and first-class calls.
    assert_eq!(
        shape_count("sq = function(x) x * x; cube(sq(3) == 9 ? 1 : 0);", STABLE),
        1
    );
...
fn stable_list_comprehensions() {
    assert_eq!(
        shape_count(
            "v = [for (i = [0:2]) i * 2]; cube(v[2] == 4 ? 1 : 0);",
            STABLE
        ),
        1
    );
    assert_eq!(
        shape_count(
            "v = [for (i = [0:2]) if (i % 2 == 0) i]; cube(len(v) == 2 ? 1 : 0);",
            STABLE
        ),
        1
    );
    assert_eq!(
        shape_count(
            "v = [for (i = [0:2]) let(j = i * 10) j]; cube(v[1] == 10 ? 1 : 0);",
            STABLE
        ),
        1
    );
    ...
    assert_eq!(
        shape_count(
            "v = [for (i = 0; i < 3; i = i + 1) i * 2]; cube(v[2] == 4 ? 1 : 0);",
            STABLE
        ),
        1
    );
    ...
    assert_eq!(
        shape_count(
            "v = [0, [for (i = [1:2]) i], 3]; cube(len(v) == 4 ? 1 : 0);",
            STABLE
        ),
        1
    );
...
fn stable_ranges_and_undef_semantics() {
    ...
    assert!(
        evaluation
            .warnings
            .contains(&"begin is greater than the end, but step is positive".to_string())
    );
    assert_eq!(evaluation.shapes.len(), 1);
    ...
    assert!(
        evaluation
            .warnings
            .contains(&"Invalid range bounds produce undef".to_string())
    );
    ...
    assert!(
        evaluation
            .warnings
            .iter()
            .any(|w| w.starts_with("Undefined operation (undefined + number)"))
    );
    // Vector arithmetic is elementwise with min length.
    assert_eq!(
        shape_count("v = [1,2,3] + [10,20]; cube(v[1] == 22 ? 1 : 0);", STABLE),
        1
    );
    // String and vector indexing.
    assert_eq!(
        shape_count("cube(\"hello\"[1] == \"e\" ? 1 : 0);", STABLE),
        1
    );
    assert_eq!(shape_count("cube([3, 4, 5].y == 4 ? 1 : 0);", STABLE), 1);
    assert_eq!(
        shape_count("r = [10:2:20]; cube(r[2] == 20 ? 1 : 0);", STABLE),
        1
    );
}
...
fn stable_echo_and_assert() {
    ...
    assert_eq!(
        eval_err("assert(false) cube(1);", STABLE),
        "Assertion 'false' failed"
    );
    assert_eq!(
    ...
    assert_eq!(
        eval_err("cube(assert(1 == 2) 1);", STABLE),
...
fn stable_modules_children_and_stack() {
    ...
    assert_eq!(
        shape_count(
            "module m() { children(0); } m() { cube(1); cube(2); }",
            STABLE
        ),
        1
    );
    assert_eq!(
        shape_count(
            "module m() { children([0, 1]); } m() { cube(1); cube(2); }",
            STABLE
        ),
        1
    );
    ...
    assert_eq!(
        shape_count(
            "module m() { if ($children == 2) cube(1); } m() { cube(1); cube(2); }",
            STABLE
        ),
        1
    );
    ...
    assert_eq!(
        shape_count(
            "x = 1; module m() { cube(x); } module caller() { x = 99; m(); } caller();",
            STABLE
        ),
        1
    );
...
fn stable_shape_algebra_merges_like_the_ts_evaluator() {
    ...
    assert_eq!(
        shape_count("translate([1,0,0]) { cube(1); cube(1); }", STABLE),
        1
    );
    ...
    assert_eq!(
        eval_err("resize([1,1,1]) { cube(1); square(1); }", STABLE),
        "resize() cannot mix 2D and 3D children"
    );
    assert_eq!(
        eval_err("minkowski() { cube(1); square(1); }", STABLE),
        "minkowski() cannot mix 2D and 3D children"
    );
}
...
fn stable_host_dependent_modules_fail_without_project() {
    for (source, message) in [
        (
            "import(\"part.stl\");",
            "import() requires an OpenSCAD project so its file is resolved inside the bounded project VFS.",
        ),
        (
    ...
    let evaluation = eval_ok(
        "x = dxf_cross(file = \"a.dxf\"); cube(is_undef(x) ? 1 : 0);",
        STABLE,
    );
    assert!(
        evaluation
            .warnings
            .contains(&"Can't open DXF file 'a.dxf'!".to_string())
    );
}
...
fn builtins_numeric_functions() {
    ...
    assert_eq!(
        shape_count(
            "cube(sign(-7) == -1 && sign(0) == 0 && sign(7) == 1 ? 1 : 0);",
            STABLE
        ),
        1
    );
    assert_eq!(
        shape_count(
            "cube(min([3, 1, 2]) == 1 && max(1, 5, 2) == 5 ? 1 : 0);",
            STABLE
        ),
        1
    );
    assert_eq!(
        shape_count("cube(min(\"b\", \"a\") == \"a\" ? 1 : 0);", STABLE),
        1
    );
    ...
    assert!(
        evaluation
            .warnings
            .contains(&"min() expects at least 1 argument".to_string())
    );
}
...
fn builtins_string_and_vector_functions() {
    assert_eq!(
        shape_count(
            "cube(str(\"a\", 1, [2], undef, true) == \"a1[2]undeftrue\" ? 1 : 0);",
            STABLE
        ),
        1
    );
    ...
    assert_eq!(
        shape_count("cube(chr([65, 66]) == \"AB\" ? 1 : 0);", STABLE),
        1
    );
    ...
    assert_eq!(
        shape_count(
            "cube(concat([1], 2, [3, 4]) == [1, 2, 3, 4] ? 1 : 0);",
            STABLE
        ),
        1
    );
    ...
    assert_eq!(
        shape_count(
            "cube(cross([1, 0, 0], [0, 1, 0]) == [0, 0, 1] ? 1 : 0);",
            STABLE
        ),
        1
    );
    assert_eq!(
        shape_count("cube(cross([1, 0], [0, 1]) == 1 ? 1 : 0);", STABLE),
        1
    );
    assert_eq!(
        shape_count("cube(version() == [2021, 1, 0] ? 1 : 0);", STABLE),
        1
    );
    assert_eq!(
        shape_count("cube(version_num() == 20210100 ? 1 : 0);", STABLE),
        1
    );
    assert_eq!(
        shape_count("cube(version_num([2019, 5]) == 20190500 ? 1 : 0);", STABLE),
        1
    );
}
...
fn builtins_lookup_and_search() {
    assert_eq!(
        shape_count(
            "cube(lookup(15, [[0, 0], [10, 100], [20, 200]]) == 150 ? 1 : 0);",
            STABLE
        ),
        1
    );
    assert_eq!(
        shape_count(
            "cube(lookup(-5, [[0, 0], [10, 100]]) == 100 ? 1 : 0);",
            STABLE
        ),
        1
    );
    assert_eq!(
        shape_count(
            "cube(search(\"ab\", \"abcabc\") == [[0, 3], [1, 4]] ? 1 : 0);",
            STABLE
        ),
        1
    );
    assert_eq!(
        shape_count("cube(search(3, [1, 2, 3, 3]) == [2, 3] ? 1 : 0);", STABLE),
        1
    );
    assert_eq!(
        shape_count(
            "cube(search(\"b\", [[\"a\", 1], [\"b\", 2]], 1) == [1] ? 1 : 0);",
            STABLE
        ),
        1
    );
...
fn builtins_predicates_and_is_undef_probe() {
    assert_eq!(
        shape_count("cube(is_undef(never_declared) ? 1 : 0);", STABLE),
        1
    );
    ...
    assert!(
        !evaluation
            .warnings
            .iter()
            .any(|w| w.contains("never_declared"))
    );
    assert_eq!(
        shape_count("cube(is_list([1]) && !is_list(\"a\") ? 1 : 0);", STABLE),
        1
    );
    assert_eq!(
        shape_count("cube(is_num(0/1) && !is_num(\"1\") ? 1 : 0);", STABLE),
        1
    );
    assert_eq!(
        shape_count("cube(is_bool(false) && is_string(\"s\") ? 1 : 0);", STABLE),
        1
    );
    assert_eq!(
        shape_count("cube(is_function(function(x) x) ? 1 : 0);", STABLE),
        1
    );
    assert_eq!(
        shape_count("f = function(x) x; cube(is_function(f) ? 1 : 0);", STABLE),
        1
    );
}
...
fn builtins_seeded_rands_are_repeatable() {
    ...
    assert_eq!(
        shape_count(
            "a = rands(0, 1, 5, 42); b = rands(0, 1, 5, 42); cube(a == b ? 1 : 0);",
            STABLE
        ),
        1
    );
    assert_eq!(
        shape_count(
            "v = rands(5, 5, 3, 1); cube(v == [5, 5, 5] ? 1 : 0);",
            STABLE
        ),
        1
    );
...
fn str_formats_function_literals_like_source() {
    assert_eq!(
        shape_count(
            "cube(str(function(x) x + 1) == \"function(x) (x + 1)\" ? 1 : 0);",
            STABLE
        ),
        1
    );
