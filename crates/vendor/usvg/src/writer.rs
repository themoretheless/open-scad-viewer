...
fn write_path(
    ...
    // Freeze non-scaling strokes at the document's intrinsic physical size.
    // Portable normalized SVG can then be resized and reimported without
    // depending on the consumer's CSS pixels-per-inch or viewport nesting.
    if !is_clip_path && path.stroke.as_ref().is_some_and(Stroke::is_non_scaling) {
        let mut fill_path = path.clone();
        fill_path.stroke = None;
        let outline = path.stroke_outline(4.0).map(|data| {
            let stroke = path.stroke.as_ref().unwrap();
            let mut outline = path.clone();
            outline.id.clear();
            outline.data = std::sync::Arc::new(data);
            outline.fill = Some(Fill {
                paint: stroke.paint.clone(),
                opacity: stroke.opacity,
                rule: FillRule::NonZero,
                context_element: stroke.context_element.clone(),
            });
            outline.stroke = None;
            outline
        });
        if path.paint_order == PaintOrder::StrokeAndFill {
            if let Some(outline) = &outline {
                write_path(outline, false, path_transform, clip_path, opt, xml);
            }
            if fill_path.fill.is_some() {
                write_path(&fill_path, false, path_transform, clip_path, opt, xml);
            }
        } else {
            if fill_path.fill.is_some() {
                write_path(&fill_path, false, path_transform, clip_path, opt, xml);
