...
fn convert_children(
    ...
    // `transform` already combines the use element's own transform with its
    // viewport/position. Install that complete transform before converting
    // children; pre-transforming parent as well would apply the element's
    // transform twice to absolute metadata (but only once when rendering).
    ...
    if let Some(mut g) =
        converter::convert_group(node, state, required, cache, parent, &|cache, g| {
            ...
            g.abs_transform = old_abs_transform.pre_concat(transform);
