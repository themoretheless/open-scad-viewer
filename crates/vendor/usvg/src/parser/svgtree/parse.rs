...
fn parse<'input>(
    ...
    let mut doc = Document {
        ...
        computed_font_size: HashMap::new(),
        dpi: options.dpi as f64,
        font_size: options.font_size as f64,
    };
    ...
    let sheets = resolve_css(xml, options.style_sheet.as_deref());
    let mut style_sheet = simplecss::StyleSheet::new();
    for css in &sheets {
        style_sheet.parse_more(css);
    }
...
pub(crate) fn parse_svg_element<'input>(
    ...
    let mut geometry_values: Vec<(AId, String, bool)> = Vec::new();
    ...
    for attr in xml_node.attributes() {
        ...
        if super::geometry::PROPERTIES.contains(&aid)
            && super::geometry::geometry_property_applies(aid.to_str(), xml_node.tag_name().name())
        {
            // Attribute d contains raw path data; CSS d contains a string/path().
            let value = if aid == AId::D {
                Some(attr.value().to_owned())
            } else {
                super::geometry::normalize_geometry_property(aid.to_str(), attr.value())
            };
            if let Some(value) = value {
                geometry_values.push((aid, value, false));
            }
            continue;
        }
        append_attribute(
    ...
    let mut insert_attribute = |aid, value: &str, important: bool| {
        if super::geometry::PROPERTIES.contains(&aid) {
            if let Some(value) = super::geometry::normalize_geometry_property(aid.to_str(), value) {
                if let Some(existing) = geometry_values.iter_mut().find(|v| v.0 == aid) {
                    if important || !existing.2 {
                        *existing = (aid, value, important);
                    }
                } else {
                    geometry_values.push((aid, value, important));
                }
            }
            return;
        }
        let vector_value = (aid == AId::VectorEffect).then(|| value.trim().to_ascii_lowercase());
        let value = vector_value.as_deref().unwrap_or(value);
        let value = if aid == AId::VectorEffect && matches!(value, "initial" | "unset" | "revert") {
            "none"
        } else {
            value
        };
        ...
        if added {
            if let Some(idx) = idx {
                ...
                // Rules are applied in ascending specificity/source order.
                // Within one author origin later important declarations win;
                // normal declarations never override an important declaration.
    ...
    let mut write_declaration = |declaration: &Declaration| {
        ...
        } else if let Some(aid) = AId::from_str(&name) {
    ...
    if let Some(value) = xml_node.attribute("style") {
        let normalized = super::geometry::normalize_css_important(value);
    ...
    let parent_font = doc
        ...
        .get(&parent_id)
        .copied()
        .unwrap_or(doc.font_size);
    let font_attr = doc.attrs[attrs_start_idx..]
        .iter_mut()
        .find(|a| a.name == AId::FontSize);
    let font_size = match font_attr {
        Some(attr) => {
            let value = computed_font_size(&attr.value, parent_font, doc.font_size, doc.dpi);
            attr.value = roxmltree::StringStorage::new_owned(&value.to_string());
            value
        }
        None => parent_font,
    };
    let mut computed_geometry = Vec::new();
    for (aid, mut value, important) in geometry_values {
        if value == "inherit" {
            value = doc
                .computed_geometry
                .get(&parent_id)
                .and_then(|values| values.iter().find(|v| v.0 == aid))
                .map(|v| v.1.clone())
                .unwrap_or_else(|| super::geometry::initial(aid).to_owned());
        } else if matches!(value.as_str(), "initial" | "unset" | "revert") {
            value = super::geometry::initial(aid).to_owned();
        } else if let Ok(length) = value.parse::<svgtypes::Length>() {
            if matches!(
                length.unit,
                svgtypes::LengthUnit::Em | svgtypes::LengthUnit::Ex
            ) {
                value = (length.number * font_size
                    / if length.unit == svgtypes::LengthUnit::Ex {
                        2.
                    } else {
                        1.
                    })
        ...
        if super::geometry::geometry_property_applies(aid.to_str(), xml_node.tag_name().name()) {
            doc.append_attribute(aid, roxmltree::StringStorage::new_owned(&value), important);
        }
        computed_geometry.push((aid, value));
    }
    ...
    doc.computed_font_size.insert(node_id, font_size);
    if !computed_geometry.is_empty() {
        doc.computed_geometry.insert(node_id, computed_geometry);
    }
    Ok(node_id)
}

fn computed_font_size(value: &str, parent: f64, initial: f64, dpi: f64) -> f64 {
    ...
    if let Ok(length) = value.parse::<svgtypes::Length>() {
        ...
        let value = match length.unit {
            ...
            Pt => n * dpi / 72.,
            Pc => n * dpi / 6.,
            Percent => n * parent / 100.,
        };
        return if value.is_finite() && value >= 0. {
            value
        } else {
            parent
        };
    ...
    match value {
        ...
        "xx-small" => initial / 1.2f64.powi(3),
        "x-small" => initial / 1.2f64.powi(2),
        "small" => initial / 1.2,
        "large" => initial * 1.2,
        "x-large" => initial * 1.2f64.powi(2),
        "xx-large" => initial * 1.2f64.powi(3),
        "smaller" => parent / 1.2,
        "larger" => parent * 1.2,
        _ => parent,
    }
...
fn append_attribute<'input>(
    ...
    let value = if aid == AId::VectorEffect && matches!(&*value, "initial" | "unset" | "revert") {
        roxmltree::StringStorage::Borrowed("none")
    } else {
        value
    };
...
fn resolve_css<'a>(xml: &'a roxmltree::Document<'a>, style_sheet: Option<&'a str>) -> Vec<String> {
    ...
    if let Some(style_sheet) = style_sheet {
        sheet.push(super::geometry::normalize_css_important(style_sheet));
    }
    ...
    for node in xml.descendants().filter(|n| n.has_tag_name("style")) {
        ...
        sheet.push(super::geometry::normalize_css_important(text));
    }
