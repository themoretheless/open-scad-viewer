...
mod parse;
pub use geometry::{
    geometry_property_applies, normalize_css_important, normalize_geometry_property,
};
mod text;
...
pub struct Document<'input> {
    ...
    computed_geometry: HashMap<NodeId, Vec<(AId, String)>>,
    computed_font_size: HashMap<NodeId, f64>,
    dpi: f64,
    font_size: f64,
}
...
impl AId {
    fn is_presentation(&self) -> bool {
        geometry::PROPERTIES.contains(self)
            || matches!(
                self,
...
fn is_non_inheritable(id: AId) -> bool {
    geometry::PROPERTIES.contains(&id)
        || matches!(
            id,
            AId::AlignmentBaseline
                | AId::BaselineShift
