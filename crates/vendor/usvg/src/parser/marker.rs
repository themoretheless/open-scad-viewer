...
fn resolve(
    ...
    let non_scaling = shape_node.attribute::<&str>(AId::VectorEffect) == Some("non-scaling-stroke")
        && marker_node.attribute::<&str>(AId::MarkerUnits) != Some("userSpaceOnUse");
    let mut host_transform = parent.abs_transform;
    host_transform.tx = 0.0;
    host_transform.ty = 0.0;
    let inverse_host = if non_scaling {
        Some(host_transform.invert()?)
    } else {
        None
    };
    let host_path;
    let path = if non_scaling {
        host_path = path.clone().transform(host_transform)?;
        &host_path
    } else {
        path
    };
    ...
    let draw_marker = |p: tiny_skia_path::Point, idx: usize| {
        ...
        let angle = match convert_orientation(marker_node) {
            ...
            MarkerOrientation::Angle(angle) if non_scaling => {
                let angle = angle.to_radians();
                let (x, y) = (angle.cos(), angle.sin());
                (host_transform.ky * x + host_transform.sy * y)
                    .atan2(host_transform.sx * x + host_transform.kx * y)
                    .to_degrees()
            }
        ...
        if let Some(inverse) = inverse_host {
            ts = inverse.pre_concat(ts);
        }
