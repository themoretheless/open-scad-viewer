...
pub(crate) fn flatten(text: &mut Text, cache: &mut Cache) -> Option<(Group, NonZeroRect)> {
    ...
    for mut child in new_children {
        // Glyph geometry is already laid out in the text element's user
        // coordinates. Retain its outer viewport transform for non-scaling
        // strokes; an identity transform would incorrectly scale text outlines.
        if let Node::Path(path) = &mut child {
            if path.stroke.as_ref().is_some_and(Stroke::is_non_scaling) {
                if let Some(updated) = Path::new(
                    path.id.clone(),
                    path.visible,
                    path.fill.clone(),
                    path.stroke.clone(),
