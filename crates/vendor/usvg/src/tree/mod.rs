...
impl Stroke {
    /// Whether the outline is constructed in the outermost SVG viewport.
...
impl Path {
    ...
    pub(crate) fn new(
        ...
        let non_scaling = stroke.as_ref().is_some_and(Stroke::is_non_scaling);
        let non_scaling_outline = if non_scaling {
            Self::outline(stroke.as_ref()?, &data, abs_transform, 1.0, false)
        } else {
            None
        };
        let stroke_bounding_box = if non_scaling {
            non_scaling_outline
                .as_ref()
                .and_then(|p| p.compute_tight_bounds())
        } else {
            Path::calculate_stroke_bbox(stroke.as_ref(), &data)
        }
        .unwrap_or(bounding_box);

        let abs_bounding_box: Rect;
        let abs_stroke_bounding_box: Rect;
        if non_scaling {
            abs_bounding_box = data
                .as_ref()
                .clone()
                .transform(abs_transform)?
                .compute_tight_bounds()?;
            abs_stroke_bounding_box = non_scaling_outline
                .and_then(|p| p.transform(abs_transform))
                .and_then(|p| p.compute_tight_bounds())
                .unwrap_or(abs_bounding_box);
        } else if abs_transform.has_skew() {
            // TODO: avoid re-alloc
    ...
    /// The centerline in the coordinate system where its stroke is constructed.
    /// Translation is omitted because it commutes with stroking and can cause
    /// precision loss for artwork translated far from the viewport origin.
    pub fn stroke_centerline(&self) -> Option<tiny_skia_path::Path> {
        let mut data = self.data.as_ref().clone();
        if self.stroke.as_ref()?.non_scaling {
            data = data.transform(self.stroke_transform())?;
        }
        Some(data)
    }

    fn stroke_transform(&self) -> Transform {
        let mut transform = self.abs_transform;
        transform.tx = 0.0;
        transform.ty = 0.0;
        transform
    }

    /// Expands dashes and stroke in the appropriate host space, returning an
    /// ordinary filled outline in this path's original user coordinate system.
    pub fn stroke_outline(&self, resolution: f32) -> Option<tiny_skia_path::Path> {
        Self::outline(
            self.stroke.as_ref()?,
            &self.data,
            self.abs_transform,
    ...
    fn outline(
        ...
        if dashed {
            if let Some(dash) = &style.dash {
                data = data.dash(dash, resolution)?;
            }
        }
        let mut outline = data.stroke(&style, resolution)?;
        if stroke.non_scaling {
            outline = outline.transform(transform.invert()?)?;
        }
        Some(outline)
    }
