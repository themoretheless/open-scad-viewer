...
impl MixedDimensionalBrep {
    pub fn new(
        ...
        if parts.is_empty() || parts.len() > MAX_PARTS {
            return Err(refuse("BREP_COMPLEX_RESOURCE_LIMIT", "Complex requires 1..64 parts"));
        }
        ...
        Ok(Self { parts, shared_faces, radial_rings, vertex_fans, lineage })
    }
    pub fn parts(&self) -> &[ComplexPart] { &self.parts }
    pub fn shared_faces(&self) -> &[SharedFace] { &self.shared_faces }
    pub fn radial_rings(&self) -> &[EdgeRadialRing] { &self.radial_rings }
    pub fn vertex_fans(&self) -> &[VertexFan] { &self.vertex_fans }
    pub fn lineage(&self) -> &[TopologyLineageRecord] { &self.lineage }
...
impl AuditedTopologyComplex {
    pub fn complex(&self) -> &MixedDimensionalBrep { &self.complex }
    pub fn certificate(&self) -> &TopologyComplexCertificate { &self.certificate }
    pub fn into_complex(self) -> MixedDimensionalBrep { self.complex }
    ...
    pub fn boundary_faces(&self) -> Vec<FaceRef> {
        let internal: BTreeSet<_> = self.complex.shared_faces.iter()
            .flat_map(|relation| relation.uses.iter().map(|u| (u.part, u.face)))
            .collect();
        let mut boundary = Vec::new();
        for (part_index, part) in self.complex.parts.iter().enumerate() {
            if !matches!(part.role, BodyRole::Solid | BodyRole::SheetShell | BodyRole::OpenShell | BodyRole::Face) {
                continue;
            }
...
fn hex_decode(text: &str) -> Result<Vec<u8>> {
    if text.len() % 2 != 0 || text.len() > 32 * 1024 * 1024 {
        return Err(refuse("BREP_COMPLEX_INTERCHANGE_REFUSED", "Invalid or oversized payload encoding"));
    }
    ...
    text.as_bytes().chunks_exact(2).map(|pair| {
        Ok((nibble(pair[0]).ok_or_else(|| refuse("BREP_COMPLEX_INTERCHANGE_REFUSED", "Invalid payload hex"))? << 4)
            | nibble(pair[1]).ok_or_else(|| refuse("BREP_COMPLEX_INTERCHANGE_REFUSED", "Invalid payload hex"))?)
    }).collect()
}
...
fn encode_complex(
    ...
    if audited.complex.parts.iter().any(|part| part.role != BodyRole::Solid) {
        return Err(refuse(
    ...
    for part in &audited.complex.parts {
        lines.push(format!("PART {} {}", part.role.as_str(), hex_encode(export(&part.model)?.as_bytes())));
    }
    for relation in &audited.complex.shared_faces {
        lines.push(format!("SHARED {}", relation.uses.iter().map(|u| format!("{},{},{}", u.part, u.face, usize::from(u.reversed))).collect::<Vec<_>>().join(";")));
    }
    for ring in &audited.complex.radial_rings {
        lines.push(format!("RADIAL {}", ring.uses.iter().map(|u| format!("{},{},{},{}", u.part, u.face, u.edge, usize::from(u.reversed))).collect::<Vec<_>>().join(";")));
    }
    for fan in &audited.complex.vertex_fans {
        lines.push(format!("FAN {},{},{} {}", fan.vertex.0, fan.vertex.1, usize::from(fan.closed),
            fan.uses.iter().map(|u| format!("{},{},{}", u.part, u.face, u.vertex)).collect::<Vec<_>>().join(";")));
    }
    lines.push("END".into());
    let text = lines.join("\n");
    if text.len() > 16 * 1024 * 1024 {
        return Err(refuse("BREP_COMPLEX_INTERCHANGE_REFUSED", "Complex interchange exceeds 16 MiB"));
    }
    Ok(text)
}

fn parse_usize(text: Option<&str>, what: &str) -> Result<usize> {
    text.ok_or_else(|| refuse("BREP_COMPLEX_INTERCHANGE_REFUSED", format!("Missing {what}")))?
        .parse().map_err(|_| refuse("BREP_COMPLEX_INTERCHANGE_REFUSED", format!("Invalid {what}")))
}
fn parse_bool_usize(text: Option<&str>, what: &str) -> Result<bool> {
    match parse_usize(text, what)? {
        0 => Ok(false),
        1 => Ok(true),
        _ => Err(refuse("BREP_COMPLEX_INTERCHANGE_REFUSED", format!("Invalid {what}"))),
    }
}
fn parse_role(text: &str) -> Result<BodyRole> {
    match text {
        "solid" => Ok(BodyRole::Solid),
        _ => Err(refuse("BREP_COMPLEX_INTERCHANGE_REFUSED", "Only solid direct interchange parts are admitted")),
    }
}
...
fn decode_complex(
    ...
    if lines.next() != Some(format!("OSV-CLOSE-TOPOLOGY-{format}/1").as_str()) {
        return Err(refuse("BREP_COMPLEX_INTERCHANGE_REFUSED", "Complex interchange header mismatch"));
    }
    ...
    for line in lines {
        ...
        match fields.next() {
            Some("PART") => {
                let role = parse_role(fields.next().ok_or_else(|| refuse("BREP_COMPLEX_INTERCHANGE_REFUSED", "Missing part role"))?)?;
                let payload = String::from_utf8(hex_decode(fields.next().unwrap_or_default())?)
                    .map_err(|_| refuse("BREP_COMPLEX_INTERCHANGE_REFUSED", "Payload is not UTF-8"))?;
                parts.push(ComplexPart { role, model: import(&payload)? });
            }
            Some("SHARED") => {
                let uses = fields.next().unwrap_or_default().split(';').map(|entry| {
                    let mut f = entry.split(',');
                    Ok(FaceRef { part: parse_usize(f.next(), "part")?, face: parse_usize(f.next(), "face")?, reversed: parse_bool_usize(f.next(), "orientation")? })
            ...
            Some("RADIAL") => {
                let uses = fields.next().unwrap_or_default().split(';').map(|entry| {
                    let mut f = entry.split(',');
                    Ok(EdgeUseRef { part: parse_usize(f.next(), "part")?, face: parse_usize(f.next(), "face")?, edge: parse_usize(f.next(), "edge")?, reversed: parse_bool_usize(f.next(), "orientation")? })
                }).collect::<Result<Vec<_>>>()?;
            ...
            Some("FAN") => {
                ...
                let vertex = (parse_usize(h.next(), "fan part")?, parse_usize(h.next(), "fan vertex")?);
                let closed = parse_bool_usize(h.next(), "fan closure")?;
                let uses = fields.next().unwrap_or_default().split(';').map(|entry| {
                    let mut f = entry.split(',');
                    Ok(VertexUseRef { part: parse_usize(f.next(), "part")?, face: parse_usize(f.next(), "face")?, vertex: parse_usize(f.next(), "vertex")? })
                }).collect::<Result<Vec<_>>>()?;
                vertex_fans.push(VertexFan { vertex, uses, closed });
            }
            _ => return Err(refuse("BREP_COMPLEX_INTERCHANGE_REFUSED", "Unknown complex interchange record")),
        }
    }
    if !ended || parts.is_empty() {
        return Err(refuse("BREP_COMPLEX_INTERCHANGE_REFUSED", "Truncated complex interchange"));
    }
    MixedDimensionalBrep::new(parts, shared_faces, radial_rings, vertex_fans, vec![])?.audit()
}

fn interchange_certificate(audited: &AuditedTopologyComplex, capability: &'static str) -> ComplexInterchangeCertificate {
...
pub fn export_complex_step(audited: &AuditedTopologyComplex) -> Result<(String, ComplexInterchangeCertificate)> {
    let text = encode_complex(audited, "STEP", |model| crate::export_step_v4(model).map(|v| v.0))?;
    Ok((text, interchange_certificate(audited, CLOSE_TOPOLOGY_STEP_CAPABILITY)))
}
pub fn import_complex_step(text: &str) -> Result<(AuditedTopologyComplex, ComplexInterchangeCertificate)> {
    let audited = decode_complex(text, "STEP", |payload| crate::import_step_v4(payload).map(|v| v.0))?;
    let certificate = interchange_certificate(&audited, CLOSE_TOPOLOGY_STEP_CAPABILITY);
    Ok((audited, certificate))
}
pub fn export_complex_iges(audited: &AuditedTopologyComplex) -> Result<(String, ComplexInterchangeCertificate)> {
    let text = encode_complex(audited, "IGES", |model| crate::export_iges_v2(model).map(|v| v.0))?;
    Ok((text, interchange_certificate(audited, CLOSE_TOPOLOGY_IGES_CAPABILITY)))
}
pub fn import_complex_iges(text: &str) -> Result<(AuditedTopologyComplex, ComplexInterchangeCertificate)> {
    let audited = decode_complex(text, "IGES", |payload| crate::import_iges_v2(payload).map(|v| v.0))?;
...
fn face_surface_key(model: &Model, face: usize) -> Result<String> {
    ...
    value_codec::to_string(&face.surface)
        .map_err(|_| refuse("BREP_COMPLEX_INCIDENCE_REFUSED", "Cannot encode shared face carrier"))
}

fn edge_curve_key(model: &Model, edge: usize) -> Result<String> {
    let edge = model.edges.get(edge)
        .ok_or_else(|| refuse("BREP_COMPLEX_INCIDENCE_REFUSED", "Unknown radial edge"))?;
    value_codec::to_string(&edge.curve)
        .map_err(|_| refuse("BREP_COMPLEX_INCIDENCE_REFUSED", "Cannot encode radial edge carrier"))
}
...
fn audit_complex(complex: MixedDimensionalBrep) -> Result<AuditedTopologyComplex> {
        ...
        match part.role {
            BodyRole::Solid => {
                if part.model.bodies.is_empty() || part.model.shells.iter().any(|s| !s.closed) {
                    return Err(refuse("BREP_COMPLEX_ROLE_REFUSED", "Solid part requires closed body shells"));
                }
                solid_audits.push(LocallyValidatedModel::new(part.model.clone())?.audit()?.certificate().clone());
            }
            BodyRole::SheetShell => {
                sheet_count += 1;
                if part.model.shells.is_empty() || part.model.shells.iter().any(|s| !s.closed) || !part.model.bodies.is_empty() {
                    return Err(refuse("BREP_COMPLEX_ROLE_REFUSED", "Sheet shell requires closed shell topology without a solid body"));
                }
            }
            BodyRole::OpenShell => {
                open_shell_count += 1;
                if part.model.shells.is_empty() || part.model.shells.iter().all(|s| s.closed) || !part.model.bodies.is_empty() {
                    return Err(refuse("BREP_COMPLEX_ROLE_REFUSED", "Open shell requires an open shell and no solid body"));
                }
            }
            BodyRole::Face if part.model.faces.len() != 1 || !part.model.bodies.is_empty() => {
                return Err(refuse("BREP_COMPLEX_ROLE_REFUSED", "Face part requires exactly one face and no body"));
            }
            BodyRole::Wire if !part.model.faces.is_empty() || !part.model.shells.is_empty() || !part.model.bodies.is_empty() => {
                return Err(refuse("BREP_COMPLEX_ROLE_REFUSED", "Wire role cannot own faces, shells, or bodies"));
            }
            BodyRole::Compound => {
                return Err(refuse("BREP_COMPLEX_ROLE_REFUSED", "Nested compound parts are not admitted"));
            }
            _ => {}
        }
    }
    ...
    for relation in &complex.shared_faces {
        if !(2..=8).contains(&relation.uses.len()) {
            return Err(refuse("BREP_COMPLEX_BRANCH_REFUSED", "Shared face valence must be 2..8"));
        }
        let first = relation.uses[0];
        let first_part = complex.parts.get(first.part)
            .ok_or_else(|| refuse("BREP_COMPLEX_INCIDENCE_REFUSED", "Unknown shared-face part"))?;
        if first_part.role != BodyRole::Solid {
            return Err(refuse("BREP_COMPLEX_ROLE_REFUSED", "Shared material faces require solid cell owners"));
        }
        let carrier = face_surface_key(&first_part.model, first.face)?;
        let mut orientations = BTreeSet::new();
        for use_ in &relation.uses {
            let part = complex.parts.get(use_.part)
                .ok_or_else(|| refuse("BREP_COMPLEX_INCIDENCE_REFUSED", "Unknown shared-face part"))?;
            if part.role != BodyRole::Solid || face_surface_key(&part.model, use_.face)? != carrier {
                return Err(refuse("BREP_COMPLEX_CARRIER_REFUSED", "Shared faces require one exact surface carrier"));
            }
            if !shared_uses.insert((use_.part, use_.face)) {
                return Err(refuse("BREP_COMPLEX_INCIDENCE_REFUSED", "Face appears in multiple shared-face relations"));
            }
            orientations.insert(use_.reversed);
        }
        if relation.uses.len() == 2 && orientations.len() != 2 {
            return Err(refuse("BREP_COMPLEX_ORIENTATION_REFUSED", "Two-cell shared face orientations must oppose"));
        }
    }
    ...
    for ring in &complex.radial_rings {
        if !(3..=MAX_RADIAL_USES).contains(&ring.uses.len()) {
            return Err(refuse("BREP_COMPLEX_BRANCH_REFUSED", "Non-manifold radial ring valence must be 3..16"));
        }
        ...
        for use_ in &ring.uses {
            let part = complex.parts.get(use_.part)
                .ok_or_else(|| refuse("BREP_COMPLEX_INCIDENCE_REFUSED", "Unknown radial part"))?;
            if part.model.faces.get(use_.face).is_none() || edge_curve_key(&part.model, use_.edge)? != carrier {
                return Err(refuse("BREP_COMPLEX_CARRIER_REFUSED", "Radial uses require one exact curve carrier"));
            }
            if !ring_seen.insert((use_.part, use_.face, use_.edge)) {
                return Err(refuse("BREP_COMPLEX_INCIDENCE_REFUSED", "Duplicate use in radial ring"));
            }
        }
        let canonical = ring.uses.iter().map(|u| (u.part, u.face, u.edge)).min().unwrap();
        if !radial_keys.insert(canonical) {
            return Err(refuse("BREP_COMPLEX_INCIDENCE_REFUSED", "Duplicate radial ring"));
        }
    }
    ...
    for fan in &complex.vertex_fans {
        if fan.uses.is_empty() || fan.uses.len() > MAX_RADIAL_USES {
            return Err(refuse("BREP_COMPLEX_BRANCH_REFUSED", "Vertex fan requires 1..16 uses"));
        }
        let part = complex.parts.get(fan.vertex.0)
            .ok_or_else(|| refuse("BREP_COMPLEX_INCIDENCE_REFUSED", "Unknown fan part"))?;
        if part.model.vertices.get(fan.vertex.1).is_none() {
            return Err(refuse("BREP_COMPLEX_INCIDENCE_REFUSED", "Unknown fan vertex"));
        }
        for use_ in &fan.uses {
            let owner = complex.parts.get(use_.part)
                .ok_or_else(|| refuse("BREP_COMPLEX_INCIDENCE_REFUSED", "Unknown fan use part"))?;
            if owner.model.faces.get(use_.face).is_none() || owner.model.vertices.get(use_.vertex).is_none() {
                return Err(refuse("BREP_COMPLEX_INCIDENCE_REFUSED", "Unknown vertex fan incidence"));
            }
            if !fan_membership.insert((fan.vertex, *use_)) {
                return Err(refuse("BREP_COMPLEX_INCIDENCE_REFUSED", "Vertex use repeated in disconnected fans"));
            }
        }
    }
    ...
    for relation in &complex.lineage {
        if relation.parents.is_empty() && relation.children.is_empty() {
            return Err(refuse("BREP_COMPLEX_NAMING_REFUSED", "Empty complex lineage relation"));
        }
        if !matches!(relation.operation.as_str(), "persist" | "split" | "merge" | "share" | "decompose" | "recompose") {
            return Err(refuse("BREP_COMPLEX_NAMING_REFUSED", "Unsupported complex lineage operation"));
        }
    }
    let boundary_face_count = complex.parts.iter().enumerate()
        .flat_map(|(p, part)| part.model.shells.iter().flat_map(move |s| s.faces.iter().map(move |u| (p, u.face))))
        .filter(|face| !shared_uses.contains(face))
        .count();
    let naming_complete = complex.parts.iter().all(|part| part.model.persistent_naming_complete());
    if !naming_complete {
        return Err(refuse("BREP_COMPLEX_NAMING_REFUSED", "Every part requires complete persistent naming"));
    }
    ...
    Ok(AuditedTopologyComplex { complex, certificate })
}
...
impl ExactParameterPartition {
    pub fn validate(&self) -> Result<()> {
        if self.child_domain_bits.len() < 2 || self.child_domain_bits.len() > 16 {
            return Err(refuse("BREP_HEAL_PARTITION_REFUSED", "Split/merge requires 2..16 child intervals"));
        }
        let decode = |bits: u64| f64::from_bits(bits);
        let [parent_lo, parent_hi] = self.parent_domain_bits.map(decode);
        if self.carrier_definition.is_empty() || !(parent_lo.is_finite() && parent_hi.is_finite() && parent_lo < parent_hi) {
            return Err(refuse("BREP_HEAL_PARTITION_REFUSED", "Invalid exact carrier or parent interval"));
        }
        ...
        for interval in &self.child_domain_bits {
            ...
            if lo.to_bits() != cursor.to_bits() || !hi.is_finite() || !(lo < hi) {
                return Err(refuse("BREP_HEAL_PARTITION_REFUSED", "Child intervals must exactly and contiguously partition parent"));
            }
            cursor = hi;
        }
        if cursor.to_bits() != parent_hi.to_bits() {
            return Err(refuse("BREP_HEAL_PARTITION_REFUSED", "Child intervals do not exactly cover parent"));
        }
        Ok(())
    }
}
...
pub fn certify_complex_heal(
    ...
    if plan.context_canonical.is_empty() || plan.correspondences.is_empty() || plan.correspondences.len() > 256 {
        return Err(refuse("BREP_HEAL_PLAN_INVALID", "Complex heal requires one context and 1..256 correspondences"));
    }
    if !plan.per_entity_limit_mm.is_finite() || !plan.cumulative_limit_mm.is_finite()
        || plan.per_entity_limit_mm <= 0. || plan.cumulative_limit_mm <= 0.
    {
        return Err(refuse("BREP_HEAL_BUDGET_EXCEEDED", "Heal budgets must be finite and positive"));
    }
    let mut touched = BTreeSet::new();
    let mut cumulative = 0.;
    for correspondence in &plan.correspondences {
        if correspondence.entities.len() < 2 || correspondence.entities.len() > 16
            || correspondence.context_path.is_empty() || correspondence.provenance.is_empty()
            || !correspondence.displacement_mm.is_finite() || correspondence.displacement_mm < 0.
            || correspondence.displacement_mm > plan.per_entity_limit_mm
        {
            return Err(refuse("BREP_HEAL_CORRESPONDENCE_REFUSED", "Invalid local correspondence, hierarchy, provenance, or budget"));
        }
        let mut local = BTreeSet::new();
        for (part, id) in &correspondence.entities {
            let owner = audited.complex.parts.get(*part)
                .ok_or_else(|| refuse("BREP_HEAL_CORRESPONDENCE_REFUSED", "Unknown correspondence part"))?;
            ...
            if id.kind() != expected || !owner.model.1.change_set.nodes.contains_key(id) || !local.insert((*part, *id)) {
                return Err(refuse("BREP_HEAL_CORRESPONDENCE_REFUSED", "Correspondence identity is stale, duplicate, or wrong-kind"));
            }
        ...
        if cumulative > plan.cumulative_limit_mm {
            return Err(refuse("BREP_HEAL_BUDGET_EXCEEDED", "Cumulative physical displacement budget exceeded"));
        }
    }
    for partition in &plan.partitions { partition.validate()?; }
...
pub fn complex_relation_id(role: &str, parents: &[TopoId], children: &[TopoId]) -> TopoId {
    ...
    TopoId::derive(TopoKind::Body, CLOSE_TOPOLOGY_CAPABILITY, role, "complex-relation", &bytes)
}
...
mod tests {
    ...
    fn solid(model: Model) -> ComplexPart { ComplexPart { role: BodyRole::Solid, model } }

    #[test]
    fn solid_typestate_is_not_weakened() {
        let one = MixedDimensionalBrep::new(vec![solid(cuboid([0.;3], [1.;3]).unwrap())], vec![], vec![], vec![], vec![])
            .unwrap().audit().unwrap();
        one.try_into_globally_audited_solid_set().unwrap();

        let a = cuboid([0.;3], [1.;3]).unwrap();
        let b = a.clone();
        let relation = SharedFace { uses: vec![
            FaceRef { part: 0, face: 0, reversed: false },
            FaceRef { part: 1, face: 0, reversed: true },
        ]};
        let mixed = MixedDimensionalBrep::new(vec![solid(a), solid(b)], vec![relation], vec![], vec![], vec![])
            .unwrap().audit().unwrap();
        ...
        assert_eq!(mixed.try_into_globally_audited_solid_set().unwrap_err().code, "BREP_SOLID_AUDIT_REFUSED");
    }
    ...
    fn radial_fans_and_orientation_are_checked() {
        ...
        let parts = vec![solid(model.clone()), solid(model.clone()), solid(model.clone())];
        let ring = EdgeRadialRing { uses: (0..3).map(|part| EdgeUseRef { part, face: 0, edge: 0, reversed: part % 2 == 0 }).collect() };
        let fan = VertexFan {
            vertex: (0, model.edges[0].vertices[0]),
            uses: vec![VertexUseRef { part: 0, face: 0, vertex: model.edges[0].vertices[0] }],
            closed: false,
        };
        let audited = MixedDimensionalBrep::new(parts, vec![], vec![ring], vec![fan], vec![
            TopologyLineageRecord { operation: "share".into(), entity_kind: "edge".into(), parents: vec![edge], children: vec![edge, face] }
        ]).unwrap().audit().unwrap();
        assert_eq!(audited.certificate().max_radial_valence, 3);
        assert_eq!(audited.certificate().vertex_fan_count, 1);

        let bad = SharedFace { uses: vec![
            FaceRef { part: 0, face: 0, reversed: false },
            FaceRef { part: 1, face: 0, reversed: false },
        ]};
        assert_eq!(MixedDimensionalBrep::new(
            vec![solid(model.clone()), solid(model)], vec![bad], vec![], vec![], vec![]
        ).unwrap().audit().unwrap_err().code, "BREP_COMPLEX_ORIENTATION_REFUSED");
    }
    ...
    fn multi_cell_heal_budgets_partitions_and_atomic_refusal() {
        ...
        let audited = MixedDimensionalBrep::new(
            vec![solid(model.clone()), solid(model.clone())], vec![], vec![], vec![], vec![]
        ).unwrap().audit().unwrap();
        ...
        assert_eq!(certify_complex_heal(&audited, &over).unwrap_err().code, "BREP_HEAL_BUDGET_EXCEEDED");
    }
    ...
    fn sheet_solid_compound_transform_permutation_and_resources() {
        ...
        let placed = crate::transform::affine(&source, [
            [0., -1., 0., 4.], [1., 0., 0., 5.], [0., 0., 1., 6.], [0., 0., 0., 1.],
        ]).unwrap();
        ...
        let audited = MixedDimensionalBrep::new(
            vec![
                ComplexPart { role: BodyRole::SheetShell, model: sheet },
                solid(placed),
            ],
            vec![], vec![], vec![], vec![],
        ).unwrap().audit().unwrap();
        assert_eq!((audited.certificate().sheet_count, audited.certificate().solid_cell_count), (1, 1));
        assert_eq!(audited.boundary_faces().len(), 12);

        let over = (0..=MAX_PARTS).map(|_| solid(source.clone())).collect();
        assert_eq!(MixedDimensionalBrep::new(over, vec![], vec![], vec![], vec![]).unwrap_err().code, "BREP_COMPLEX_RESOURCE_LIMIT");
        assert_eq!(import_complex_step("OSV-CLOSE-TOPOLOGY-STEP/1\nBOGUS\nEND").unwrap_err().code, "BREP_COMPLEX_INTERCHANGE_REFUSED");
    }
}
