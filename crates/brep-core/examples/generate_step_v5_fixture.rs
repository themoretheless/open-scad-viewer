use brep_core::{Body,Model,bicubic_open_face,cylinder,export_step_v5,export_step_v6,export_step_v8,
...
fn main(){
    ...
    let kind=kind.strip_prefix("v6-").or_else(||kind.strip_prefix("v8-")).or_else(||kind.strip_prefix("v9-")).unwrap_or(&kind).to_string();
    let base_kind=match kind.as_str(){"nested-placement"|"assembly"|"metre"|"inch"|"periodic-cylinder"=>"cylinder","periodic-cone"=>"cone","periodic-torus"=>"torus","pole-sphere"=>"sphere","pole-cone"=>"pole-cone",other=>other};
    let model=if kind=="open-shell"{open_shell()}else if kind=="multiple-void"{multiple_void_model()}else{match base_kind{
    ...
    let mut text=if v9{export_step_v9(&model).expect("STEP /9 export").0}
        else if v8{export_step_v8(&model).expect("STEP /8 export").0}
        else if v6{export_step_v6(&model).expect("STEP /6 export").0}
        else{export_step_v5(&model).expect("STEP /5 export").0};
    if kind=="metre"{text=text.replace("SI_UNIT(.MILLI.,.METRE.)","SI_UNIT($,.METRE.)")}
    if kind=="inch"{
        let unit=entity_id(&text,"SI_UNIT(");
        text=text.replace(&format!("GLOBAL_UNIT_ASSIGNED_CONTEXT((#{unit}))"),"GLOBAL_UNIT_ASSIGNED_CONTEXT((#60002))");
...
");
    ...
    if kind=="assembly"{
        ...
        let definition_line=text.lines().find(|line|line.starts_with(&format!("#{definition}="))).unwrap();
...
",refs[0],refs[1]);
    ...
    if let Some(path)=std::env::args().nth(2){std::fs::write(path,text).expect("write fixture")}
    else{print!("{text}")}
}
...
fn open_shell()->Model{
    ...
    bicubic_open_face(Surface{degree_u:3,degree_v:3,knots_u:vec![0.,0.,0.,0.,1.,1.,1.,1.],
        knots_v:vec![0.,0.,0.,0.,1.,1.,1.,1.],control_points,weights:vec![vec![1.;4];4],
        periodic_u:false,periodic_v:false}).unwrap()
}

fn entity_id(text:&str,marker:&str)->usize{
    let line=text.lines().find(|line|line.starts_with('#')&&line.contains(marker)).expect("fixture entity");
    line[1..line.find('=').unwrap()].parse().unwrap()
}
fn refs_in_line(line:&str)->Vec<usize>{
    line.split('#').skip(2).filter_map(|part|part.split(|ch:char|!ch.is_ascii_digit()).next()?.parse().ok()).collect()
}
...
fn multiple_void_model()->Model{
    ...
    model.rebuild_topology_ids();model.validate().unwrap();model
}
