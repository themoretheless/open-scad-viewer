...
use crate::{Result, check, curve::Curve, numeric, resource, surface::{Axis, Surface}};
...
fn copy_context(value: &ToleranceContext) -> Result<ToleranceContext> {
    value_codec::from_value(value_codec::to_value(value)
...
fn bernstein_split(coefficients: &[f64]) -> (Vec<f64>, Vec<f64>) {
    ...
    while rows.last().unwrap().len() > 1 {
        rows.push(rows.last().unwrap().array_windows().map(|[a, b]| (a + b) * 0.5).collect());
    }
...
fn sign_variations(coefficients: &[f64]) -> usize {
    ...
    for &coefficient in coefficients {
        let sign = if coefficient > 0. { 1 } else if coefficient < 0. { -1 } else { 0 };
        if sign != 0 {
...
fn stationary_coefficients(curve: &Curve, query: &[f64]) -> Vec<f64> {
    ...
    let homogeneous: Vec<Vec<f64>> = curve.control_points.iter().zip(&curve.weights).map(|(point, weight)| {
        point.iter().map(|coordinate| coordinate * weight)
            .chain(std::iter::once(*weight)).collect()
    }).collect();
    let derivative: Vec<Vec<f64>> = homogeneous.array_windows().map(|[a, b]| {
        a.iter().zip(b).map(|(x, y)| p as f64 * (y - x)).collect()
    }).collect();
    let mut result = vec![0.; 3 * p];
    for axis in 0..dimension {
        let offset: Vec<f64> = homogeneous.iter()
            .map(|value| value[axis] - query[axis] * value[dimension]).collect();
        let first = bernstein_product(
            &derivative.iter().map(|value| value[axis]).collect::<Vec<_>>(),
            &homogeneous.iter().map(|value| value[dimension]).collect::<Vec<_>>(),
        );
        let second = bernstein_product(
            &homogeneous.iter().map(|value| value[axis]).collect::<Vec<_>>(),
            &derivative.iter().map(|value| value[dimension]).collect::<Vec<_>>(),
        );
        let numerator: Vec<f64> = first.into_iter().zip(second).map(|(a, b)| a - b).collect();
        for (target, value) in result.iter_mut().zip(bernstein_product(&offset, &numerator)) {
            *target += value;
        }
    }
    result
...
fn isolate_stationary(coefficients: Vec<f64>, floor: f64) -> (Vec<RootBox>, bool) {
    ...
    let mut pending = vec![RootBox { lo: 0., hi: 1., variation: sign_variations(&coefficients), coefficients }];
    let mut roots = Vec::new();
    let mut work = 0_usize;
    while let Some(node) = pending.pop() {
        if node.variation == 0 { continue; }
        work += 1;
        if node.variation == 1 && node.hi - node.lo <= floor.min(2_f64.powi(-48)).max(2_f64.powi(-52)) {
            roots.push(node);
            continue;
        }
        ...
        pending.push(RootBox { lo: middle, hi: node.hi, variation: sign_variations(&right), coefficients: right });
        pending.push(RootBox { lo: node.lo, hi: middle, variation: sign_variations(&left), coefficients: left });
    }
...
fn distance(point: &[f64], query: &[f64]) -> f64 {
    point.iter().zip(query).map(|(a, b)| (a - b) * (a - b)).sum::<f64>().sqrt()
}

fn point_box_distance(min: &[f64], max: &[f64], point: &[f64]) -> f64 {
    point.iter().enumerate().map(|(axis, coordinate)| {
        if *coordinate < min[axis] { min[axis] - coordinate }
        else if *coordinate > max[axis] { coordinate - max[axis] } else { 0. }
        ...
        for i in u.saturating_sub(nu)..=u.min(mu) {
            for j in v.saturating_sub(nv)..=v.min(mv) {
                sum += binomial(mu, i) * binomial(nu, u-i) / binomial(mu+nu, u)
                    * binomial(mv, j) * binomial(nv, v-j) / binomial(mv+nv, v)
                    * first[i][j] * second[u-i][v-j];
            }
        }
        sum
    }).collect()).collect()
...
fn surface_normal_bounds(surface: &Surface, iu: usize, iv: usize) -> Vec<[f64; 2]> {
    ...
    let controls = (iu-p..=iu).map(|u| (iv-q..=iv).map(|v| {
        let w = surface.weights[u][v];
        [surface.control_points[u][v][0]*w, surface.control_points[u][v][1]*w,
         surface.control_points[u][v][2]*w, w]
    }).collect::<Vec<_>>()).collect::<Vec<_>>();
    let du = (0..p).map(|u| (0..=q).map(|v| {
        std::array::from_fn::<_,4,_>(|a| p as f64*(controls[u+1][v][a]-controls[u][v][a]))
    }).collect::<Vec<_>>()).collect::<Vec<_>>();
    let dv = (0..=p).map(|u| (0..q).map(|v| {
        std::array::from_fn::<_,4,_>(|a| q as f64*(controls[u][v+1][a]-controls[u][v][a]))
    }).collect::<Vec<_>>()).collect::<Vec<_>>();
    let net = |component: usize| controls.iter().map(|row| row.iter().map(|x| x[component]).collect()).collect::<Vec<Vec<f64>>>();
    let dunet = |component: usize| du.iter().map(|row| row.iter().map(|x| x[component]).collect()).collect::<Vec<Vec<f64>>>();
    let dvnet = |component: usize| dv.iter().map(|row| row.iter().map(|x| x[component]).collect()).collect::<Vec<Vec<f64>>>();
    let w = net(3);
    let wu = dunet(3);
    let wv = dvnet(3);
    let mut nu = Vec::new();
    ...
    for axis in 0..3 {
        ...
        nu.push(xu_w.iter().zip(x_wu).map(|(a,b)| a.iter().zip(b).map(|(x,y)| x-y).collect()).collect::<Vec<Vec<f64>>>());
        let xv_w = tensor_product(&dvnet(axis), &w);
        let x_wv = tensor_product(&net(axis), &wv);
        nv.push(xv_w.iter().zip(x_wv).map(|(a,b)| a.iter().zip(b).map(|(x,y)| x-y).collect()).collect::<Vec<Vec<f64>>>());
    }
    [(1,2),(2,0),(0,1)].into_iter().map(|(a,b)| {
        let first = tensor_product(&nu[a], &nv[b]);
        let second = tensor_product(&nu[b], &nv[a]);
        interval(first.into_iter().flatten().zip(second.into_iter().flatten()).map(|(x,y)| x-y))
    }).collect()
}
...
fn fully_bezier_surface(surface: &Surface) -> Result<Surface> {
    ...
    for axis in [Axis::U, Axis::V] {
        let (degree, knots, domain) = match axis {
            Axis::U => (output.degree_u, output.knots_u.clone(), [output.knots_u[output.degree_u], output.knots_u[output.control_points.len()]]),
            Axis::V => (output.degree_v, output.knots_v.clone(), [output.knots_v[output.degree_v], output.knots_v[output.control_points[0].len()]]),
        };
        let mut unique = Vec::<(f64,usize)>::new();
        for knot in knots {
            if knot <= domain[0] || knot >= domain[1] { continue; }
            if let Some(last) = unique.last_mut().filter(|last| last.0 == knot) { last.1 += 1; }
            else { unique.push((knot, 1)); }
        }
        for (knot, multiplicity) in unique {
            if multiplicity < degree {
                output = output.edit_axis(axis, |curve| curve.insert(knot, degree-multiplicity))?;
            }
        }
    }
    Ok(output)
...
pub fn certify_surface(surface: &Surface, tolerance: Option<ToleranceContext>) -> Result<Value> {
    ...
    let localization = localize_singularities(&bezier, &cells, tolerance.parametric_bounds().floor)?;
    Ok(json!({
        "version":"nurbs-foundation/1",
        "kind":"surface",
        "rounding":"binary64-nextafter-outward",
...
pub fn project_curve(
    ...
    for segment in curve.decompose()? {
        ...
        let analytic_linear = if c.degree == 1 {
            ...
            let direction=last.iter().zip(first).map(|(x,y)|x-y).collect::<Vec<_>>();
            let denominator=direction.iter().map(|value|value*value).sum::<f64>();
            if denominator>0. {
                let lambda=point.iter().zip(first).zip(&direction).map(|((x,o),d)|(x-o)*d).sum::<f64>()/denominator;
                if lambda>0.&&lambda<1. {
                    Some(lambda*c.weights[0]/(c.weights[1]*(1.-lambda)+lambda*c.weights[0]))
                } else {None}
            } else {None}
        } else {None};
        let (roots, continuum) = if analytic_linear.is_some() {(vec![],false)}
            else {isolate_stationary(stationary_coefficients(c, point), floor / (b-a))};
        ...
        intervals.extend(roots.iter().map(|root| (root.lo, root.hi,
            if root.variation == 1 { "simple_stationary" } else { "multiple_or_clustered_stationary" })));
        for (lo, hi, classification) in intervals {
            ...
            let restricted = if lo == hi { None } else {
                let mut piece = c.clone();
                if hi < 1. { piece = piece.split(a+(b-a)*hi)?[0].clone(); }
                if lo > 0. {
                    let mapped = a+(b-a)*lo;
            ...
            let (min,max) = restricted.as_ref().map(|value| box_of(&value.control_points))
    ...
    let mut kept: Vec<Value> = candidates.into_iter().filter(|candidate|
        candidate["distanceLower"].as_f64().unwrap() <= next_up(best_upper)).collect();
    kept.sort_by(|a,b| a["distanceUpper"].as_f64().unwrap().total_cmp(&b["distanceUpper"].as_f64().unwrap())
        .then(a["parameterInterval"][0].as_f64().unwrap().total_cmp(&b["parameterInterval"][0].as_f64().unwrap())));
    let winner = kept.iter().enumerate().find(|(i,candidate)| kept.iter().enumerate().all(|(j,other)|
        i==&j || candidate["distanceUpper"].as_f64().unwrap() < other["distanceLower"].as_f64().unwrap()));
    let status = if stationary_continuum { "nonunique" } else if winner.is_some() { "unique" }
        else if kept.len()>1 { "nonunique_or_unresolved" } else { "isolated_candidate" };
    Ok(json!({
...
pub fn project_surface(
    ...
    surface.validate()?;
    check(point.iter().all(|value| value.is_finite()), "Projection point must be finite")?;
    let tolerance = context(tolerance);
    let nu = surface.control_points.len();
    let nv = surface.control_points[0].len();
    let domain = [surface.knots_u[surface.degree_u], surface.knots_u[nu],
        surface.knots_v[surface.degree_v], surface.knots_v[nv]];
    let make_box = |bounds: [f64;4]| -> Result<SurfaceBox> {
        ...
        Ok(SurfaceBox { bounds, lower: next_down(point_box_distance(&min,&max,&point).max(0.)),
            upper: next_up(distance(&evaluated,&point)), point:evaluated })
    };
    ...
    while subdivisions < MAX_CERTIFICATE_CELLS {
        ...
        let Some((index,_)) = active.iter().enumerate().filter(|(_,cell)|
            (cell.bounds[1]-cell.bounds[0]).max(cell.bounds[3]-cell.bounds[2]) > target)
            .max_by(|(_,a),(_,b)| (a.bounds[1]-a.bounds[0]).max(a.bounds[3]-a.bounds[2])
                .total_cmp(&(b.bounds[1]-b.bounds[0]).max(b.bounds[3]-b.bounds[2]))) else { break };
        if active.len()+1 >= MAX_CERTIFICATE_CELLS { break; }
        let cell = active.swap_remove(index);
        let [u0,u1,v0,v1] = cell.bounds;
        let children = if u1-u0 >= v1-v0 {
            let middle=(u0+u1)*0.5;
            [make_box([u0,middle,v0,v1])?,make_box([middle,u1,v0,v1])?]
        } else {
            let middle=(v0+v1)*0.5;
            [make_box([u0,u1,v0,middle])?,make_box([u0,u1,middle,v1])?]
        };
    ...
    active.retain(|cell| cell.lower <= best);
    active.sort_by(|a,b| a.bounds[0].total_cmp(&b.bounds[0]).then(a.bounds[2].total_cmp(&b.bounds[2])));
    let boundaries = [
    ...
    ].into_iter().map(|(edge,curve)| project_curve(&curve,&point,Some(copy_context(&tolerance)?))
        .map(|certificate|json!({"edge":edge,"certificate":certificate}))).collect::<Result<Vec<_>>>()?;
    let mut boxes = active;
    let affine_uniqueness=if surface.degree_u==1&&surface.degree_v==1&&nu==2&&nv==2
        && surface.weights.iter().flatten().all(|weight|*weight==surface.weights[0][0]) {
        let origin=&surface.control_points[0][0];
        let du=surface.control_points[1][0].iter().zip(origin).map(|(x,o)|x-o).collect::<Vec<_>>();
        let dv=surface.control_points[0][1].iter().zip(origin).map(|(x,o)|x-o).collect::<Vec<_>>();
        let closure=surface.control_points[1][1].iter().zip(origin).zip(&du).zip(&dv)
            .all(|(((x,o),u),v)|(*x-*o-*u-*v).abs()<=64.*f64::EPSILON);
        let a=du.iter().map(|x|x*x).sum::<f64>();let b=du.iter().zip(&dv).map(|(x,y)|x*y).sum::<f64>();
        let c=dv.iter().map(|x|x*x).sum::<f64>();let determinant=a*c-b*b;
        let rhsu=point.iter().zip(origin).zip(&du).map(|((x,o),d)|(x-o)*d).sum::<f64>();
        let rhsv=point.iter().zip(origin).zip(&dv).map(|((x,o),d)|(x-o)*d).sum::<f64>();
        let su=(rhsu*c-rhsv*b)/determinant;let sv=(rhsv*a-rhsu*b)/determinant;
        (closure&&determinant>0.&&su>=0.&&su<=1.&&sv>=0.&&sv<=1.).then_some((su,sv,determinant))
    }else{None};
    let mut uniqueness_proof = None;
    let mut status = if boxes.len()==1 {"isolated_candidate"} else {"nonunique_or_unresolved"};
    if let Some((su,sv,determinant))=affine_uniqueness {
        let u=domain[0]+(domain[1]-domain[0])*su;let v=domain[2]+(domain[3]-domain[2])*sv;
        let projected=surface.evaluate_validated(u,v)?.point;let d=next_up(distance(&projected,&point));
        boxes=vec![SurfaceBox{bounds:[u,u,v,v],lower:next_down(d),upper:d,point:projected}];
        status="unique";
        uniqueness_proof=Some(json!({"method":"strict-convex-affine-Gram","gramDeterminantLower":next_down(determinant)}));
    } else if let Some(proof)=prove_surface_projection_uniqueness(surface,&point,&mut boxes,best,target)? {
        status="unique";
        uniqueness_proof=Some(proof);
        best=boxes.iter().map(|cell|cell.upper).fold(f64::INFINITY,f64::min);
    }
...
fn solve(mut matrix: Vec<Vec<f64>>, mut values: Vec<Vec<f64>>) -> Result<Vec<Vec<f64>>> {
    let n = matrix.len();
    for column in 0..n {
        let pivot = (column..n).max_by(|&a,&b| matrix[a][column].abs().total_cmp(&matrix[b][column].abs())).unwrap();
        numeric(matrix[pivot][column].abs() > 64.*f64::EPSILON, "Reduction interpolation system is singular")?;
        matrix.swap(column,pivot); values.swap(column,pivot);
        let divisor=matrix[column][column];
        for value in &mut matrix[column][column..] { *value/=divisor; }
        for value in &mut values[column] { *value/=divisor; }
        for row in 0..n {
            if row==column { continue; }
            let factor=matrix[row][column];
            for j in column..n { matrix[row][j]-=factor*matrix[column][j]; }
            let pivot_values=values[column].clone();
            for (value,pivot_value) in values[row].iter_mut().zip(pivot_values) { *value-=factor*pivot_value; }
        }
    }
    Ok(values)
}
...
fn refit_curve(source: &Curve, degree: usize, knots: Vec<f64>) -> Result<Curve> {
    ...
    check(count>degree && count<=256, "Reduction would produce an invalid control count")?;
    let parameters=(0..count).map(|i| if degree==0 {knots[i]} else {
        knots[i+1..=i+degree].iter().sum::<f64>()/degree as f64
    }).collect::<Vec<_>>();
    let matrix=parameters.iter().map(|&u| crate::curve::basis(degree,&knots,count,u,false).map(|basis|basis.basis)).collect::<Result<Vec<_>>>()?;
    let dimension=source.control_points[0].len();
    let mut values=Vec::new();
    for &u in &parameters {
        let basis=crate::curve::basis(source.degree,&source.knots,source.control_points.len(),u,source.periodic)?.basis;
        let weight=basis.iter().zip(&source.weights).map(|(b,w)|b*w).sum::<f64>();
        let point=source.evaluate(u)?.point;
        values.push(point.into_iter().map(|x|x*weight).chain(std::iter::once(weight)).collect());
    }
    let homogeneous=solve(matrix,values)?;
    let weights=homogeneous.iter().map(|value|value[dimension]).collect::<Vec<_>>();
    numeric(weights.iter().all(|weight|*weight>=1e-12&&*weight<=1e12), "Reduction produced inadmissible weights")?;
    let control_points=homogeneous.iter().map(|value|value[..dimension].iter().map(|x|x/value[dimension]).collect()).collect();
    let output=Curve {degree,knots,control_points,weights,periodic:false};
    output.validate()?;
    Ok(output)
}

fn exact_curve(first: &Curve, second: &Curve) -> bool {
    first.degree==second.degree && first.knots==second.knots && first.control_points==second.control_points
        && first.weights==second.weights && first.periodic==second.periodic
}
fn exact_surface(first: &Surface, second: &Surface) -> bool {
    first.degree_u==second.degree_u && first.degree_v==second.degree_v
        && first.knots_u==second.knots_u && first.knots_v==second.knots_v
        && first.control_points==second.control_points && first.weights==second.weights
        && first.periodic_u==second.periodic_u && first.periodic_v==second.periodic_v
}

pub fn remove_curve_knot(
    source: &Curve, knot: f64, max_error: f64, tolerance: Option<ToleranceContext>
) -> Result<Value> {
    source.validate()?;
    check(!source.periodic, "Certified removal currently requires non-periodic storage")?;
    check(max_error.is_finite()&&max_error>=0., "Maximum error must be finite and nonnegative")?;
    let domain=source.domain();
    check(knot>domain[0]&&knot<domain[1], "Only interior knots can be removed")?;
    let Some(index)=source.knots.iter().position(|value|*value==knot) else { return Err(crate::input("Knot is absent")); };
    let mut knots=source.knots.clone(); knots.remove(index);
    ...
    let error=if exact {0.} else {periodic_error(source,&reconstructed)?};
    let accepted=error<=max_error;
    let tolerance=context(tolerance);
    Ok(json!({"curve":if accepted {candidate} else {source.clone()},"certificate":{
...
pub fn reduce_curve_degree(
    source: &Curve, degree: usize, max_error: f64, tolerance: Option<ToleranceContext>
) -> Result<Value> {
    source.validate()?;
    check(!source.periodic, "Certified reduction currently requires non-periodic storage")?;
    check(degree>=1&&degree<source.degree, "Target degree must be in [1, source degree)")?;
    check(max_error.is_finite()&&max_error>=0., "Maximum error must be finite and nonnegative")?;
    ...
    for _ in 0..remove {
        ...
        while index>0 {
            index-=1;
            if previous!=Some(knots[index]) { previous=Some(knots[index]); knots.remove(index); }
        }
    }
...
pub fn reduce_surface_axis(
    source: &Surface, axis: Axis, operation: &str, parameter: f64, degree: usize,
    ...
    let candidate=source.edit_axis(axis, |curve| {
        if operation=="remove" {
            ...
            let index=knots.iter().position(|value|*value==parameter).ok_or_else(||crate::input("Knot is absent"))?;
            knots.remove(index); refit_curve(curve,curve.degree,knots)
        } else {
            let mut knots=curve.knots.clone();
            for _ in 0..curve.degree-degree {
                let mut index=knots.len(); let mut previous=None;
                while index>0 {index-=1;if previous!=Some(knots[index]){previous=Some(knots[index]);knots.remove(index);}}
            }
            refit_curve(curve,degree,knots)
        }
    })?;
    let reconstructed=candidate.edit_axis(axis, |curve| if operation=="remove" {
        curve.insert(parameter,1)
    } else {
        curve.elevate(match axis {Axis::U=>source.degree_u,Axis::V=>source.degree_v})
    })?;
    let exact=exact_surface(source,&reconstructed);
    let first=source.control_points.iter().flatten().cloned().collect::<Vec<_>>();
    let second=reconstructed.control_points.iter().flatten().cloned().collect::<Vec<_>>();
    let (min,max)=box_of(&first.iter().chain(&second).cloned().collect::<Vec<_>>());
    let error=if exact {0.} else {next_up(min.iter().zip(max).map(|(a,b)|(b-a)*(b-a)).sum::<f64>().sqrt())};
    let accepted=error<=max_error;
    let tolerance=context(tolerance);
...
fn periodic_knots(active: &[f64], degree: usize) -> Vec<f64> {
    ...
    let mut knots=active[active.len()-1-degree..active.len()-1].iter().map(|value|value-period).collect::<Vec<_>>();
...
fn refit_periodic(source: &Curve, degree: usize, active: Vec<f64>) -> Result<Curve> {
    ...
    check(unique>degree&&unique+degree<=256, "Periodic edit exceeds control resources")?;
    let knots=periodic_knots(&active,degree);
    let period=active[unique]-active[0];
    let parameters=(0..unique).map(|i|active[0]+period*(i as f64+0.5)/unique as f64).collect::<Vec<_>>();
    let matrix=parameters.iter().map(|&u| {
        let basis=crate::curve::basis(degree,&knots,unique+degree,u,true)?.basis;
        Ok((0..unique).map(|column|basis.iter().enumerate()
            .filter(|(index,_)|index%unique==column).map(|(_,value)|value).sum()).collect())
    }).collect::<Result<Vec<Vec<f64>>>>()?;
    let dimension=source.control_points[0].len();
    let values=parameters.iter().map(|&u| {
        let basis=crate::curve::basis(source.degree,&source.knots,source.control_points.len(),u,true)?.basis;
        let weight=basis.iter().zip(&source.weights).map(|(b,w)|b*w).sum::<f64>();
        let point=source.evaluate(u)?.point;
        Ok(point.into_iter().map(|value|value*weight).chain(std::iter::once(weight)).collect())
    }).collect::<Result<Vec<Vec<f64>>>>()?;
    let homogeneous=solve(matrix,values)?;
    let weights=homogeneous.iter().map(|value|value[dimension]).collect::<Vec<_>>();
    numeric(weights.iter().all(|weight|*weight>=1e-12&&*weight<=1e12), "Periodic edit produced inadmissible weights")?;
    let points=homogeneous.iter().map(|value|value[..dimension].iter().map(|x|x/value[dimension]).collect::<Vec<_>>()).collect::<Vec<_>>();
    let mut control_points=points.clone(); control_points.extend(points[..degree].iter().cloned());
    let mut wrapped_weights=weights.clone(); wrapped_weights.extend(weights[..degree].iter().copied());
    let curve=Curve {degree,knots,control_points,weights:wrapped_weights,periodic:true};
    curve.validate()?;
    Ok(curve)
}
...
fn seam_certificate(curve: &Curve) -> Result<Value> {
    ...
    let c1=match (&first.d1,&last.d1){(Some(x),Some(y))=>Some(residual(x,y)),_=>None};
    let c2=match (&first.d2,&last.d2){(Some(x),Some(y))=>Some(residual(x,y)),_=>None};
...
fn periodic_error(source:&Curve,target:&Curve)->Result<f64>{
    ...
    let speed_upper=|curve:&Curve|->Result<f64>{
        ...
        for segment in curve.decompose()? {
            ...
            let homogeneous=c.control_points.iter().zip(&c.weights).map(|(point,weight)|
                point.iter().map(|coordinate|coordinate*weight).chain(std::iter::once(*weight)).collect::<Vec<_>>()).collect::<Vec<_>>();
            let derivative=homogeneous.array_windows().map(|[a,b]|a.iter().zip(b)
                .map(|(x,y)|c.degree as f64*(y-x)).collect::<Vec<_>>()).collect::<Vec<_>>();
            let w=homogeneous.iter().map(|value|value[dimension]).collect::<Vec<_>>();
            let dw=derivative.iter().map(|value|value[dimension]).collect::<Vec<_>>();
            let denominator=interval(w.iter().copied());
            let components=(0..dimension).map(|axis|{
                let first=bernstein_product(&derivative.iter().map(|value|value[axis]).collect::<Vec<_>>(),&w);
                let second=bernstein_product(&homogeneous.iter().map(|value|value[axis]).collect::<Vec<_>>(),&dw);
                first.into_iter().zip(second).map(|(x,y)|(x-y).abs()).fold(0.,f64::max)/(denominator[0]*denominator[0])
            }).collect::<Vec<_>>();
            maximum=maximum.max(components.iter().map(|value|value*value).sum::<f64>().sqrt());
        }
        Ok(next_up(maximum))
    };
    ...
    for i in 0..=1024 {
        ...
        error=error.max(distance(&source.evaluate(u)?.point,&target.evaluate(u)?.point));
    }
    Ok(next_up(error+lipschitz*(b-a)/2048.))
}

fn periodic_candidate(source:&Curve,operation:&str,parameter:f64,degree:usize)->Result<Curve>{
    let mut active=periodic_active_knots(source);
    let target_degree=match operation {
        "insert"=>{check(parameter>=active[0]&&parameter<*active.last().unwrap(),"Periodic insertion must use the half-open fundamental domain")?;
            active.insert(active.partition_point(|value|*value<=parameter),parameter);source.degree},
        "remove"=>{let index=active.iter().position(|value|*value==parameter&&*value>active[0]&&*value<*active.last().unwrap())
            .ok_or_else(||crate::input("Removable periodic knot is absent"))?;active.remove(index);source.degree},
        "elevate"=>{check(degree>source.degree&&degree<=25,"Periodic elevation target is invalid")?;
            ...
            for value in original.iter().rev(){
                ...
                for _ in 0..delta {active.insert(index,*value);}
            }
            degree},
        "reduce"=>{check(degree>=1&&degree<source.degree,"Periodic reduction target is invalid")?;
            for _ in 0..source.degree-degree {
                let distinct=active.iter().copied().fold(Vec::<f64>::new(),|mut values,value|{
                    if values.last()!=Some(&value){values.push(value)} values});
...
pub fn edit_periodic_curve(
    source:&Curve, operation:&str, parameter:f64, degree:usize, max_error:f64,
    tolerance:Option<ToleranceContext>
)->Result<Value>{
    source.validate()?;
    check(source.periodic,"Wrapped periodic edit requires periodic storage")?;
    check(max_error.is_finite()&&max_error>=0.,"Maximum error must be finite and nonnegative")?;
    ...
    let tolerance=context(tolerance);
    Ok(json!({"curve":output,"certificate":{"version":"nurbs-foundation/3","operation":operation,
...
pub fn edit_periodic_surface(
    source:&Surface,axis:Axis,operation:&str,parameter:f64,degree:usize,max_error:f64,
    tolerance:Option<ToleranceContext>
)->Result<Value>{
    source.validate()?;
    let periodic=match axis {Axis::U=>source.periodic_u,Axis::V=>source.periodic_v};
    check(periodic,"Selected surface axis is not periodic")?;
    let candidate=source.edit_axis(axis,|curve|periodic_candidate(curve,operation,parameter,degree))?;
    let mut error=0_f64;
    let samples=32;
    for i in 0..=samples {
        let parameter_other=match axis {
            Axis::U=>source.knots_v[source.degree_v]+(source.knots_v[source.control_points[0].len()]-source.knots_v[source.degree_v])*i as f64/samples as f64,
            Axis::V=>source.knots_u[source.degree_u]+(source.knots_u[source.control_points.len()]-source.knots_u[source.degree_u])*i as f64/samples as f64,
        };
        let fixed=match axis {Axis::U=>Axis::V,Axis::V=>Axis::U};
        error=error.max(periodic_error(&source.iso(fixed,parameter_other)?,&candidate.iso(fixed,parameter_other)?)?);
    }
    let controls=source.control_points.iter().flatten().chain(candidate.control_points.iter().flatten()).cloned().collect::<Vec<_>>();
    let (minimum,maximum)=box_of(&controls);
    error=error.max(next_up(minimum.iter().zip(maximum).map(|(a,b)|(b-a)*(b-a)).sum::<f64>().sqrt()));
    let accepted=error<=max_error;
    let output=if accepted{candidate}else{source.clone()};
    ...
    let tolerance=context(tolerance);
    Ok(json!({"surface":output,"certificate":{"version":"nurbs-foundation/3",
...
pub fn split_periodic_curve(source:&Curve,parameter:f64,tolerance:Option<ToleranceContext>)->Result<Value>{
    source.validate()?;
    check(source.periodic,"Periodic split requires periodic storage")?;
    let [a,b]=source.domain();
    check(parameter>a&&parameter<b,"Periodic split must be strictly inside the fundamental domain")?;
    let pieces=[source.trim(parameter,b)?,source.trim(a,parameter)?];
    let tolerance=context(tolerance);
    Ok(json!({"curves":pieces,"certificate":{"version":"nurbs-foundation/3",
...
fn map_pieces(mapping:&Value)->Result<Vec<MapPiece>>{
    let values=mapping["pieces"].as_array().ok_or_else(||crate::input("Reparameterization requires pieces"))?;
    check(!values.is_empty()&&values.len()<=64,"Piecewise reparameterization needs 1..64 pieces")?;
    values.iter().map(|piece|{
        let domain:[f64;2]=value_codec::from_value(piece["domain"].clone()).map_err(|e|crate::input(e.to_string()))?;
        let range:[f64;2]=value_codec::from_value(piece["range"].clone()).map_err(|e|crate::input(e.to_string()))?;
        let controls:Vec<f64>=value_codec::from_value(piece["controlValues"].clone()).map_err(|e|crate::input(e.to_string()))?;
        let weights:Vec<f64>=value_codec::from_value(piece["weights"].clone()).map_err(|e|crate::input(e.to_string()))?;
        check((2..=26).contains(&controls.len())&&controls.len()==weights.len(),"Map piece needs 2..26 matched controls and weights")?;
        check(domain[0]<domain[1]&&range[0]<range[1]&&domain.iter().chain(range.iter()).all(|x|x.is_finite()),"Map domains and ranges must increase finitely")?;
        check(weights.iter().all(|w|w.is_finite()&&*w>=1e-12&&*w<=1e12),"Map weights must be positive and bounded")?;
        check(controls.first()==Some(&range[0])&&controls.last()==Some(&range[1]),"Map endpoint controls must equal the declared range")?;
        Ok(MapPiece{domain,range,values:controls,weights})
    }).collect()
}
fn evaluate_map_piece(piece:&MapPiece,u:f64)->f64{
    ...
    let mut numerator=0.;let mut denominator=0.;
    for i in 0..=degree {
        let basis=binomial(degree,i)*t.powi(i as i32)*(1.-t).powi((degree-i) as i32);
        numerator+=basis*piece.weights[i]*piece.values[i];denominator+=basis*piece.weights[i];
    }
    numerator/denominator
}
fn map_derivative_coefficients(piece:&MapPiece)->Vec<f64>{
    let homogeneous=piece.values.iter().zip(&piece.weights).map(|(x,w)|[x*w,*w]).collect::<Vec<_>>();
    let degree=piece.values.len()-1;
    let derivative=homogeneous.array_windows().map(|[a,b]|[
        degree as f64*(b[0]-a[0]),degree as f64*(b[1]-a[1])]).collect::<Vec<_>>();
    let first=bernstein_product(&derivative.iter().map(|x|x[0]).collect::<Vec<_>>(),&piece.weights);
    let second=bernstein_product(&homogeneous.iter().map(|x|x[0]).collect::<Vec<_>>(),
        &derivative.iter().map(|x|x[1]).collect::<Vec<_>>());
    first.into_iter().zip(second).map(|(x,y)|(x-y)/(piece.domain[1]-piece.domain[0])).collect()
}
fn evaluate_mapping(mapping:&Value,u:f64)->Result<f64>{
    if let Some(composition)=mapping.get("composition").and_then(Value::as_array){
        check(!composition.is_empty()&&composition.len()<=16,"Mapping composition needs 1..16 factors")?;
        return composition.iter().try_fold(u,|parameter,factor|evaluate_mapping(factor,parameter));
    }
    let pieces=map_pieces(mapping)?;
    let piece=pieces.iter().find(|piece|u>=piece.domain[0]&&u<=piece.domain[1])
        .ok_or_else(||crate::input("Reparameterized query lies outside the mapping domain"))?;
    Ok(evaluate_map_piece(piece,u))
}
pub fn certify_reparameterization(mapping:&Value,tolerance:Option<ToleranceContext>)->Result<Value>{
    if let Some(composition)=mapping.get("composition").and_then(Value::as_array){
        check(!composition.is_empty()&&composition.len()<=16,"Mapping composition needs 1..16 factors")?;
        let factors=composition.iter().map(|factor|certify_reparameterization(factor,None)).collect::<Result<Vec<_>>>()?;
        for pair in composition.windows(2){
            let first=map_pieces(&pair[0])?;let second=map_pieces(&pair[1])?;
            check(first.last().unwrap().range==second.first().unwrap().domain,
                "Composed mapping ranges and domains must match exactly")?;
        }
        let tolerance=context(tolerance);
        return Ok(json!({"version":"nurbs-foundation/3","mapping":mapping.clone(),
    ...
    for pair in pieces.windows(2){
        check(pair[0].domain[1]==pair[1].domain[0]&&pair[0].range[1]==pair[1].range[0],
            "Piecewise mapping must be contiguous in domain and range")?;
    }
    let certificates=pieces.iter().map(|piece|{
        let derivative=map_derivative_coefficients(piece);
        let bounds=interval(derivative.iter().copied());
        numeric(bounds[0]>0.,"Rational map derivative is not certified strictly positive")?;
        Ok(json!({"domain":piece.domain,"range":piece.range,"derivativeNumeratorBounds":bounds,
    ...
    let tolerance=context(tolerance);
    Ok(json!({"version":"nurbs-foundation/3","mapping":mapping.clone(),"classification":"certified_strictly_monotone",
        "pieces":certificates,"composition":"exact-semantic-evaluation","inverse":"interval-bisection-certified",
        "evidence":tolerance_evidence(&tolerance)}))
}
pub fn evaluate_reparameterized_curve(curve:&Curve,mapping:&Value,u:f64,tolerance:Option<ToleranceContext>)->Result<Value>{
    curve.validate()?;
    ...
    Ok(json!({"version":"nurbs-foundation/3","parameter":u,"sourceParameter":source_parameter,
...
fn chord_parameters(points:&[Vec<f64>])->Vec<f64>{
    ...
    for pair in points.windows(2){parameters.push(parameters.last().unwrap()+distance(&pair[0],&pair[1]));}
    let total=*parameters.last().unwrap();
    if total==0. {(0..points.len()).map(|i|i as f64/(points.len()-1) as f64).collect()}
    else {parameters.into_iter().map(|value|value/total).collect()}
}
pub fn fit_curve_points(
    points:Vec<Vec<f64>>,control_count:usize,tolerance:Option<ToleranceContext>
)->Result<Value>{
    check(points.len()>=2&&points.len()<=4096,"Curve fitting needs 2..4096 data sites")?;
    let dimension=points[0].len();
    check((dimension==2||dimension==3)&&points.iter().all(|p|p.len()==dimension&&p.iter().all(|x|x.is_finite())),
        "Curve fitting data must be finite 2D or 3D points")?;
    check((2..=26).contains(&control_count)&&control_count<=points.len(),"Fit control count must be 2..26 and no larger than site count")?;
    let degree=control_count-1;
    let parameters=chord_parameters(&points);
    let design=parameters.iter().map(|&t|(0..control_count).map(|i|binomial(degree,i)*t.powi(i as i32)*(1.-t).powi((degree-i) as i32)).collect::<Vec<_>>()).collect::<Vec<_>>();
    let matrix=(0..control_count).map(|i|(0..control_count).map(|j|design.iter().map(|row|row[i]*row[j]).sum()).collect()).collect();
    let rhs=(0..control_count).map(|i|(0..dimension).map(|axis|design.iter().zip(&points).map(|(row,p)|row[i]*p[axis]).sum()).collect()).collect();
    let controls=solve(matrix,rhs)?;
    let curve=Curve{degree,knots:std::iter::repeat_n(0.,degree+1).chain(std::iter::repeat_n(1.,degree+1)).collect(),
        control_points:controls,weights:vec![1.;control_count],periodic:false};
    curve.validate()?;
    let residual=parameters.iter().zip(&points).map(|(&u,point)|distance(&curve.evaluate(u).unwrap().point,point)).fold(0.,f64::max);
    let tolerance=context(tolerance);
    Ok(json!({"curve":curve,"certificate":{"version":"nurbs-foundation/3","classification":"approximate_fit",
...
pub fn interpolate_surface_grid(
    points:Vec<Vec<[f64;3]>>,fitting:bool,tolerance:Option<ToleranceContext>
)->Result<Value>{
    check(points.len()>=2&&points.len()<=32&&points[0].len()>=2&&points[0].len()<=32,
        "Surface interpolation grid must be 2..32 by 2..32")?;
    let nv=points[0].len();
    check(points.iter().all(|row|row.len()==nv&&row.iter().flatten().all(|x|x.is_finite())),
        "Surface interpolation grid must be rectangular and finite")?;
    let axis_knots=|count:usize|{
        std::iter::repeat_n(0.,2).chain((1..count-1).map(|i|i as f64/(count-1) as f64)).chain(std::iter::repeat_n(1.,2)).collect::<Vec<_>>()
    };
    let control_points=points.iter().map(|row|row.iter().map(|point|point.to_vec()).collect()).collect::<Vec<Vec<Vec<f64>>>>();
    let surface=Surface{degree_u:1,degree_v:1,knots_u:axis_knots(points.len()),knots_v:axis_knots(nv),
        weights:vec![vec![1.;nv];points.len()],control_points,periodic_u:false,periodic_v:false};
    surface.validate()?;
    let tolerance=context(tolerance);
    Ok(json!({"surface":surface,"certificate":{"version":"nurbs-foundation/3",
...
fn interval_add(a:[f64;2],b:[f64;2])->[f64;2]{[next_down(a[0]+b[0]),next_up(a[1]+b[1])]}
fn interval_sub(a:[f64;2],b:[f64;2])->[f64;2]{[next_down(a[0]-b[1]),next_up(a[1]-b[0])]}
fn interval_contains_zero(a:[f64;2])->bool{a[0]<=0.&&a[1]>=0.}
fn interval_width(a:[f64;2])->f64{a[1]-a[0]}

fn surface_jet_bounds(surface:&Surface,bounds:[f64;4])->Result<([[f64;2];3],[[f64;2];3],[[f64;2];3])>{
    let samples=5;
    ...
    for i in 0..=samples {
        for j in 0..=samples {
            ...
            let (su,sv)=evaluation.first_derivatives().ok_or_else(||crate::numeric_err("Surface jet unavailable"))?;
            for axis in 0..3 {
                point[axis][0]=point[axis][0].min(evaluation.point[axis]);
                point[axis][1]=point[axis][1].max(evaluation.point[axis]);
                du[axis][0]=du[axis][0].min(su[axis]);du[axis][1]=du[axis][1].max(su[axis]);
                dv[axis][0]=dv[axis][0].min(sv[axis]);dv[axis][1]=dv[axis][1].max(sv[axis]);
            }
        }
    }
    let patch=surface.trim(bounds)?;
    ...
    let (min,max)=box_of(&controls);
    for axis in 0..3 {
        point[axis]=[next_down(point[axis][0].min(min[axis])),next_up(point[axis][1].max(max[axis]))];
...
fn krawczyk_unique_surface_root(surface:&Surface,point:&[f64;3],bounds:[f64;4],floor:f64)->Result<Option<[f64;2]>>{
    let width_u=bounds[1]-bounds[0];let width_v=bounds[3]-bounds[2];
    if width_u.max(width_v)>floor.max(2_f64.powi(-20)) { return Ok(None); }
    let center=[(bounds[0]+bounds[1])*0.5,(bounds[2]+bounds[3])*0.5];
    let evaluation=surface.evaluate_validated(center[0],center[1])?;
    let Some((su,sv))=evaluation.first_derivatives() else { return Ok(None); };
    let residual=[
        evaluation.point.iter().zip(point).zip(su.iter()).map(|((s,q),d)|(s-q)*d).sum::<f64>(),
        evaluation.point.iter().zip(point).zip(sv.iter()).map(|((s,q),d)|(s-q)*d).sum::<f64>(),
    ];
    let (image,du,dv)=surface_jet_bounds(surface,bounds)?;
    let offset=[interval_sub(image[0],[point[0],point[0]]),interval_sub(image[1],[point[1],point[1]]),
        interval_sub(image[2],[point[2],point[2]])];
    let fu=offset.iter().zip(du.iter()).map(|(o,d)|interval_mul(*o,*d)).reduce(interval_add).unwrap();
    let fv=offset.iter().zip(dv.iter()).map(|(o,d)|interval_mul(*o,*d)).reduce(interval_add).unwrap();
    ...
    let det=a*c-b*b;
    if !(det>64.*f64::EPSILON) { return Ok(None); }
    let inv=[[c/det,-b/det],[-b/det,a/det]];
    // Krawczyk: K(X)=y - C F(y) + (I - C F'(X))(X-y)
    let cy=[center[0]-(inv[0][0]*residual[0]+inv[0][1]*residual[1]),
            center[1]-(inv[1][0]*residual[0]+inv[1][1]*residual[1])];
    // Conservative contraction radius using Gram inverse and F enclosure widths.
    let radius_u=next_up((inv[0][0].abs()*interval_width(fu)+inv[0][1].abs()*interval_width(fv))*0.5+width_u*0.25);
    let radius_v=next_up((inv[1][0].abs()*interval_width(fu)+inv[1][1].abs()*interval_width(fv))*0.5+width_v*0.25);
    let k_box=[cy[0]-radius_u,cy[0]+radius_u,cy[1]-radius_v,cy[1]+radius_v];
    let contracts=k_box[0]>=bounds[0]&&k_box[1]<=bounds[1]&&k_box[2]>=bounds[2]&&k_box[3]<=bounds[3]
        && (k_box[1]-k_box[0])<width_u && (k_box[3]-k_box[2])<width_v;
    ...
    if contracts&&contains_root&&cy[0]>=bounds[0]&&cy[0]<=bounds[1]&&cy[1]>=bounds[2]&&cy[1]<=bounds[3] {
        Ok(Some(cy))
    } else { Ok(None) }
}

fn prove_surface_projection_uniqueness(
    surface:&Surface,point:&[f64;3],boxes:&mut Vec<SurfaceBox>,best:f64,floor:f64
)->Result<Option<Value>>{
    if boxes.is_empty() { return Ok(None); }
    // Strict global lower/upper separation among overlapping boxes.
    let winner=boxes.iter().enumerate().find(|(i,cell)|
        boxes.iter().enumerate().all(|(j,other)|i==&j||cell.upper<other.lower));
    if let Some((index,_))=winner {
        let chosen=boxes.swap_remove(index);
        *boxes=vec![chosen];
        return Ok(Some(json!({"method":"strict-global-distance-interval-separation","winnerIndex":0})));
    }
    // Krawczyk isolation on the currently best box, with all others excluded by lower bounds.
    let Some((index,_))=boxes.iter().enumerate().min_by(|(_,a),(_,b)|a.upper.total_cmp(&b.upper)) else { return Ok(None); };
    if let Some(root)=krawczyk_unique_surface_root(surface,point,boxes[index].bounds,floor)? {
        ...
        let excluded=boxes.iter().enumerate().filter(|(j,other)|*j!=index&&other.lower>upper).count();
        if boxes.iter().enumerate().all(|(j,other)|j==index||other.lower>upper) {
            let excluded_total=boxes.len()-1;
            *boxes=vec![SurfaceBox{bounds:[root[0],root[0],root[1],root[1]],
                lower:next_down(upper),upper,point:evaluation.point}];
            return Ok(Some(json!({"method":"Krawczyk-interval-operator-with-global-lower-bound-exclusion",
...
fn classify_normal_box(bounds:&[[f64;2]])->&'static str{
    let zeros=bounds.iter().filter(|b|interval_contains_zero(**b)).count();
    let separated=bounds.iter().any(|b|b[0]>0.||b[1]<0.);
    let exact=bounds.iter().all(|b|b[0]==0.&&b[1]==0.);
    if exact {"singular_patch"}
    else if zeros==3 && bounds.iter().all(|b|interval_width(*b)<=2_f64.powi(-40)) {"isolated_singular_point"}
    else if zeros==2 && separated {"singular_curve_candidate"}
    else if separated {"certified_regular"}
    else {"unresolved"}
}
...
fn localize_singularities(surface:&Surface,cells:&[Value],floor:f64)->Result<Value>{
    ...
    for cell in cells {
        let classification=cell["normalRegularity"]["classification"].as_str().unwrap_or("");
        if matches!(classification,"singular"|"singular_or_unresolved"|"unresolved") {
            let domain_u: [f64;2]=value_codec::from_value(cell["domainU"].clone()).map_err(|e|crate::input(e.to_string()))?;
            let domain_v: [f64;2]=value_codec::from_value(cell["domainV"].clone()).map_err(|e|crate::input(e.to_string()))?;
    ...
    while let Some(bounds)=pending.pop() {
        ...
        let patch=match surface.trim(bounds) {
            Ok(value)=>value,
            Err(_)=>{ unresolved.push(json!({"parameterBox":bounds,"classification":"trim_failed"})); continue; }
        };
        // Evaluate normal numerator bounds on the single Bézier cell of the trimmed patch.
        let iu=patch.degree_u; let iv=patch.degree_v;
...
fn budget_controls(count:usize)->Result<()>{
    if count>256 { Err(resource("The result exceeds 256 control points")) } else { Ok(()) }
}

fn compose_curve_with_piece(curve:&Curve,piece:&MapPiece)->Result<Curve>{
    check(curve.degree<=8&&piece.values.len()<=9,"Admitted composition requires degree ≤8 map and curve")?;
    let [a,b]=curve.domain();
    check((piece.range[0]-a).abs()<=64.*f64::EPSILON&&(piece.range[1]-b).abs()<=64.*f64::EPSILON
        || (piece.range[0]>=a&&piece.range[1]<=b),
        "Map range must lie in the active curve domain")?;
    let restricted=if (piece.range[0]-a).abs()<=64.*f64::EPSILON&&(piece.range[1]-b).abs()<=64.*f64::EPSILON {
        curve.clone()
    } else {
        curve.trim(piece.range[0],piece.range[1])?
    };
    let segments=restricted.decompose()?;
    check(segments.len()==1,"Composition requires a single Bézier span after restriction")?;
    ...
    let map_p:Vec<f64>=piece.values.iter().zip(&piece.weights)
        .map(|(value,weight)|((value-piece.range[0])/(piece.range[1]-piece.range[0]))*weight).collect();
    ...
    for _ in 1..=n {
        ...
        one_powers.push(bernstein_product(one_powers.last().unwrap(),&map_one_minus));
    }
    ...
    for axis in 0..=dimension {
        let coeffs:Vec<f64>=if axis==dimension {
            ...
            span.control_points.iter().zip(&span.weights).map(|(p,w)|p[axis]*w).collect()
        };
    ...
    numeric(weights.iter().all(|w|*w>=1e-12&&*w<=1e12),"Composed weights left the positive window")?;
    let control_points=(0..=degree).map(|i|(0..dimension).map(|axis|homogeneous[axis][i]/weights[i]).collect()).collect();
    let domain=piece.domain;
    let curve=Curve{
        degree,
        knots:std::iter::repeat_n(domain[0],degree+1).chain(std::iter::repeat_n(domain[1],degree+1)).collect(),
        control_points,weights,periodic:false,
    };
    curve.validate()?;
    Ok(curve)
}
...
fn materialize_pieces(curve:&Curve,pieces:&[MapPiece])->Result<(Curve,usize)>{
    ...
    for next in composed.into_iter().skip(1) {
        check(result.degree==next.degree,"Piecewise composition produced unequal degrees")?;
        let left=result.evaluate(result.domain()[1])?.point;
        let right=next.evaluate(next.domain()[0])?.point;
        check(distance(&left,&right)<=1e-9,"Composed pieces are not C0 joinable")?;
        ...
        result=Curve{degree:result.degree,knots,control_points:controls,weights,periodic:false};
        result.validate()?;
    }
    Ok((result,degree_growth))
}

fn materialize_mapping_tree(curve:&Curve,mapping:&Value,depth:usize)->Result<(Curve,usize,Value)>{
    check(depth<=8,"Nested composition depth exceeds 8")?;
    if let Some(parts)=mapping.get("composition").and_then(Value::as_array) {
        check(!parts.is_empty()&&parts.len()<=8,"Nested composition needs 1..8 maps")?;
        ...
        return Ok((current,growth,json!({"kind":"nested_composition","depth":depth,"children":children})));
    }
    let pieces=map_pieces(mapping)?;
    let (result,growth)=materialize_pieces(curve,&pieces)?;
    Ok((result,growth,json!({"kind":"piecewise","pieceCount":pieces.len()})))
}

pub fn materialize_reparameterized_curve(
    curve:&Curve,mapping:&Value,tolerance:Option<ToleranceContext>
)->Result<Value>{
    curve.validate()?;
    ...
    if periodic_cover {
        ...
        check(distance(&left,&right)<=1e-9,"Periodic composition cover is not closed")?;
...
fn robust_solve(mut matrix:Vec<Vec<f64>>,mut values:Vec<Vec<f64>>)->Result<(Vec<Vec<f64>>,usize,f64)>{
    let n=matrix.len();
    ...
    for column in 0..n {
        ...
        let Some((row,candidate,magnitude))=pivot else { break; };
        if magnitude<=1e-12 { break; }
        pivot_min=pivot_min.min(magnitude);
        matrix.swap(rank,row); values.swap(rank,row);
        for r in 0..n { matrix[r].swap(column,candidate); }
        column_perm.swap(column,candidate);
        let divisor=matrix[rank][column];
        for value in &mut matrix[rank][column..] { *value/=divisor; }
        for value in &mut values[rank] { *value/=divisor; }
        for r in 0..n {
            if r==rank { continue; }
            let factor=matrix[r][column];
            for j in column..n { matrix[r][j]-=factor*matrix[rank][j]; }
            let pivot_values=values[rank].clone();
            for (value,pivot_value) in values[r].iter_mut().zip(pivot_values) { *value-=factor*pivot_value; }
        }
        rank+=1;
    }
...
pub fn fit_curve_cloud_certified(
    ...
    check(points.len()>=2&&points.len()<=4096,"Cloud curve fitting needs 2..4096 sites")?;
    let dimension=points[0].len();
    check((dimension==2||dimension==3)&&points.iter().all(|p|p.len()==dimension&&p.iter().all(|x|x.is_finite())),
        "Cloud points must be finite 2D or 3D")?;
    check((2..=26).contains(&control_count),"Cloud fit control count must be 2..26")?;
    let mut working=control_count.min(points.len());
    let parameters=chord_parameters(&points);
    let mut last_error=None;
    let (controls,rank,pivot_min)=loop {
        let degree=working-1;
        let design=parameters.iter().map(|&t|(0..working).map(|i|binomial(degree,i)*t.powi(i as i32)*(1.-t).powi((degree-i) as i32)).collect::<Vec<_>>()).collect::<Vec<_>>();
        let matrix=(0..working).map(|i|(0..working).map(|j|design.iter().map(|row|row[i]*row[j]).sum()).collect()).collect();
        let rhs=(0..working).map(|i|(0..dimension).map(|axis|design.iter().zip(&points).map(|(row,p)|row[i]*p[axis]).sum()).collect()).collect();
        match robust_solve(matrix,rhs) {
            Ok(result)=>break result,
            Err(error)=>{
                last_error=Some(error);
                if working<=2 { break (Vec::new(),0,0.); }
                working-=1;
            }
        }
    };
    ...
    let degree=working-1;
    let curve=Curve{degree,knots:std::iter::repeat_n(0.,degree+1).chain(std::iter::repeat_n(1.,degree+1)).collect(),
        control_points:controls[..working].to_vec(),weights:vec![1.;working],periodic:false};
    curve.validate()?;
    let site_error=parameters.iter().zip(&points).map(|(&u,point)|distance(&curve.evaluate(u).unwrap().point,point)).fold(0.,f64::max);
    ...
    let tolerance=context(tolerance);
    Ok(json!({"curve":curve,"certificate":{"version":"nurbs-foundation/4","classification":"approximate_cloud_fit",
...
pub fn fit_surface_cloud_certified(
    points:Vec<[f64;3]>,controls_u:usize,controls_v:usize,tolerance:Option<ToleranceContext>
)->Result<Value>{
    check(points.len()>=4&&points.len()<=4096,"Cloud surface fitting needs 4..4096 sites")?;
    check((2..=8).contains(&controls_u)&&(2..=8).contains(&controls_v),
        "Cloud surface controls per axis must be 2..8")?;
    check(points.iter().flatten().all(|x|x.is_finite()),"Cloud surface points must be finite")?;
    // PCA plane parameters as admitted unstructured domain.
    let centroid=[0,1,2].map(|axis|points.iter().map(|p|p[axis]).sum::<f64>()/points.len() as f64);
    let mut cov=[[0.;3];3];
    for point in &points {
        let d=[point[0]-centroid[0],point[1]-centroid[1],point[2]-centroid[2]];
    ...
    for axis in &mut axes {
        for _ in 0..24 {
            let mut next=[0.;3];
            for i in 0..3 { next[i]=cov[i][0]*axis[0]+cov[i][1]*axis[1]+cov[i][2]*axis[2]; }
            let norm=next.iter().map(|x|x*x).sum::<f64>().sqrt().max(f64::from_bits(1));
            *axis=next.map(|x|x/norm);
        }
    }
    let mut uv:Vec<[f64;2]>=points.iter().map(|point|{
        let d=[point[0]-centroid[0],point[1]-centroid[1],point[2]-centroid[2]];
    ...
    for parameter in &mut uv {
        parameter[0]=(parameter[0]-u_bounds[0])/(u_bounds[1]-u_bounds[0]).max(f64::from_bits(1));
        parameter[1]=(parameter[1]-v_bounds[0])/(v_bounds[1]-v_bounds[0]).max(f64::from_bits(1));
    }
    let degree_u=controls_u-1; let degree_v=controls_v-1;
    let count=controls_u*controls_v;
    let design=uv.iter().map(|parameter|{
        (0..controls_u).flat_map(|i|(0..controls_v).map(move |j|
            binomial(degree_u,i)*parameter[0].powi(i as i32)*(1.-parameter[0]).powi((degree_u-i) as i32)
            *binomial(degree_v,j)*parameter[1].powi(j as i32)*(1.-parameter[1]).powi((degree_v-j) as i32)
        )).collect::<Vec<_>>()
    }).collect::<Vec<_>>();
    let matrix=(0..count).map(|i|(0..count).map(|j|design.iter().map(|row|row[i]*row[j]).sum()).collect()).collect();
    let rhs=(0..count).map(|i|(0..3).map(|axis|design.iter().zip(&points).map(|(row,p)|row[i]*p[axis]).sum()).collect()).collect();
    let (flat,rank,pivot_min)=robust_solve(matrix,rhs)?;
    let control_points=(0..controls_u).map(|i|(0..controls_v).map(|j|flat[i*controls_v+j].clone()).collect()).collect();
    let surface=Surface{degree_u,degree_v,
        knots_u:std::iter::repeat_n(0.,degree_u+1).chain(std::iter::repeat_n(1.,degree_u+1)).collect(),
        knots_v:std::iter::repeat_n(0.,degree_v+1).chain(std::iter::repeat_n(1.,degree_v+1)).collect(),
        control_points,weights:vec![vec![1.;controls_v];controls_u],periodic_u:false,periodic_v:false};
    surface.validate()?;
    let site_error=uv.iter().zip(&points).map(|(parameter,point)|{
        distance(&surface.evaluate_validated(parameter[0],parameter[1]).unwrap().point,point)
    }).fold(0.,f64::max);
    let controls:Vec<Vec<f64>>=surface.control_points.iter().flatten().cloned().collect();
    let (min,max)=box_of(&controls);
    let diameter=next_up(min.iter().zip(max).map(|(a,b)|(b-a)*(b-a)).sum::<f64>().sqrt());
    let delta=1./((points.len() as f64).sqrt().max(1.));
    let hausdorff=next_up(site_error+diameter*delta);
    let tolerance=context(tolerance);
    Ok(json!({"surface":surface,"certificate":{"version":"nurbs-foundation/4","classification":"approximate_cloud_fit",
