...
pub(super) fn select_depth(
    ...
    if !tolerance.is_finite()
        ...
        || !primary.step.is_finite()
        || primary.step <= 0.
        || primary.step != secondary.step
        || primary.depth.len() != count
        || secondary.depth.len() != count
    ...
    for i in 0..count {
        if i % 4096 == 0 {
            cancelled(progress, "depth-selection-validate", i, count)?;
        }
        if !primary.depth[i].is_finite()
            || primary.depth[i] < 0.
            || !secondary.depth[i].is_finite()
            || secondary.depth[i] < 0.
    ...
    for i in 0..count {
        if i % 4096 == 0 {
            cancelled(progress, "depth-selection", i, count)?;
        }
...
mod tests {
    ...
    fn map(depth: Vec<f64>, confidence: f32) -> DepthMap {
        DepthMap {
            ...
            height: 1,
            step: 1.,
            confidence: vec![confidence; depth.len()],
            depth,
            neighbors: vec![1, 2],
        }
    }
...
fn estimator_work_merge_is_transactional_and_preserves_output_counts() {
    ...
    let mut a = DenseDiagnostics {
        evaluated_hypotheses: 10,
        evaluated_source_patches: 20,
        sampled_source_pixels: 30,
        estimated_maps: 4,
        vertices: 12,
        ..Default::default()
    };
    let b = DenseDiagnostics {
        evaluated_hypotheses: 7,
        evaluated_source_patches: 8,
        sampled_source_pixels: 9,
        estimated_maps: 4,
        vertices: 99,
        ..Default::default()
    };
    ...
    assert_eq!(
        (
            a.evaluated_hypotheses,
            a.evaluated_source_patches,
            a.sampled_source_pixels
        ),
        (17, 28, 39)
    );
    ...
    let b = DenseDiagnostics {
        ...
        evaluated_source_patches: 1,
        sampled_source_pixels: u64::MAX,
        ..Default::default()
    };
...
pub(super) fn retained_map_bytes(maps: &[DepthMap], outer_capacity: usize) -> Result<usize> {
    ...
    for map in maps {
        for (capacity, size) in [
            ...
            bytes = bytes
                .checked_add(capacity.checked_mul(size).ok_or_else(overflow)?)
                .ok_or_else(overflow)?;
        }
    }
    Ok(bytes)
...
pub(super) fn check_secondary_budget(
...
fn retained_capacity_and_secondary_budget_are_checked() {
    let map = DepthMap {
        ...
        height: 1,
        step: 1.,
        depth: Vec::with_capacity(20),
        confidence: Vec::with_capacity(30),
        neighbors: Vec::with_capacity(4),
    };
    let expected = 2 * std::mem::size_of::<DepthMap>()
        + map.depth.capacity() * 8
        + map.confidence.capacity() * 4
        + map.neighbors.capacity() * std::mem::size_of::<usize>();
