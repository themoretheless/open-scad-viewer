...
it('manual save writes the current draft without requiring a conflict and survives reopening', async () => {
  const factory = new IDBFactory()
  const repository = new IndexedDbWorkspaceRepository(factory)
  const persistence = new BrowserWorkspacePersistence(repository, new MemoryStorage())
  const initial = await persistence.initialize({ fallbackSource: 'cube(1);' })
  const draft = updateWorkspaceDocument(initial.document, { source: 'sphere(3);' })
  expect(persistence.hasConflict).toBe(false)
  expect((await persistence.saveCurrentDraft(draft)).saved).toBe(true)
  expect(persistence.backend).toBe('indexeddb')
  expect((await persistence.saveCurrentDraft(draft)).saved).toBe(true)
  const edited = updateWorkspaceDocument(draft, { source: 'sphere(4);' })
  expect(await persistence.save(edited)).toBe(true)
  await persistence.close()
  const reopened = new IndexedDbWorkspaceRepository(factory)
