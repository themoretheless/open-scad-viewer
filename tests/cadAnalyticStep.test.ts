...
describe('analytic STEP product seam', () => {
  it('keeps analytic peer separate from faceted cadStep', () => {
    ...
    expect(analytic).toContain('brep_nurbs_export_step_v2')
    expect(analytic).toContain('brep_nurbs_import_step_v2')
    expect(analytic).toContain('StepIdentityReport')
    expect(analytic).toContain('step-interchange/2')
