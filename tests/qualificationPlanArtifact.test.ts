...
const v26PlanPath = resolve(repositoryRoot, 'docs/qualification/semantic-manifold-g1-plan-v26.json')
const planPath = resolve(repositoryRoot, 'docs/qualification/semantic-manifold-g1-plan-v27.json')
const v28PlanPath = resolve(repositoryRoot, 'docs/qualification/semantic-manifold-g1-plan-v28.json')
const v29PlanPath = resolve(repositoryRoot, 'docs/qualification/semantic-manifold-g1-plan-v29.json')
const v34PlanPath = resolve(repositoryRoot, 'docs/qualification/semantic-manifold-g1-plan-v34.json')
...
const v26Plan = JSON.parse(readFileSync(v26PlanPath, 'utf8')) as JsonObject
const plan = JSON.parse(readFileSync(planPath, 'utf8')) as JsonObject
const v28Plan = JSON.parse(readFileSync(v28PlanPath, 'utf8')) as JsonObject
const v29Plan = JSON.parse(readFileSync(v29PlanPath, 'utf8')) as JsonObject
const v34Plan = JSON.parse(readFileSync(v34PlanPath, 'utf8')) as JsonObject
...
const FROZEN_V26_SHA256 = '0016bbc4ef57c02e17f61a9f4d1d84e21d37c444f15e061f77554bf357f12967'
...
describe('frozen-pending G1 QualificationPlan artifacts', () => {
  it('conforms to the machine-strict schema with closed object shapes', () => {
    ...
    expect(validateJsonSchema(schema, v28Plan)).toEqual([])
    expect(validateJsonSchema(schema, v29Plan)).toEqual([])
    expect(validateJsonSchema(schema, v34Plan)).toEqual([])
  })
  ...
  it('v27 preserves v26, rebinds current bytes, and keeps qualification blocked on u07', () => {
    expect(plan).toMatchObject({
      planId: 'semantic-manifold-g1-plan-v27',
      processAmendment: {
        previousPlanId: 'semantic-manifold-g1-plan-v26',
        previousPlanSha256: FROZEN_V26_SHA256,
        priorEvidenceTreatment: 'discovery-only', qualificationClaim: 'none',
      },
      executionProtocol: {
        candidateRunId: 'semantic-manifold-g1-candidate-run-v27',
        resultPath: 'output/qualification/semantic-manifold-g1-candidate-run-v27/result.json',
        priorResultsMayBeImported: false, plannedWorkUnits: 4740,
      },
    })
    expect(Object.keys(plan)).toEqual(Object.keys(v26Plan))
    for (const key of Object.keys(v26Plan)) {
      if (['planId', 'processAmendment', 'bindings', 'executionProtocol'].includes(key)) continue
      expect(plan[key], `v27 preserves ${key}`).toEqual(v26Plan[key])
    }
  ...
  it('v28 appends the V7 blend binding without claiming qualification', () => {
    expect(v28Plan).toMatchObject({
      planId: 'semantic-manifold-g1-plan-v28',
      processAmendment: {
        previousPlanId: 'semantic-manifold-g1-plan-v27',
        priorEvidenceTreatment: 'discovery-only',
        qualificationClaim: 'none',
      },
      lifecycle: {qualificationClaim: 'none'},
      executionProtocol: {
        candidateRunId: 'semantic-manifold-g1-candidate-run-v28',
        resultPath: 'output/qualification/semantic-manifold-g1-candidate-run-v28/result.json',
        priorResultsMayBeImported: false,
  ...
  it('v29 appends the V8 shell binding without claiming qualification', () => {
    expect(v29Plan).toMatchObject({
      planId: 'semantic-manifold-g1-plan-v29',
      processAmendment: {
        previousPlanId: 'semantic-manifold-g1-plan-v28',
        priorEvidenceTreatment: 'discovery-only',
        qualificationClaim: 'none',
      },
      lifecycle: {qualificationClaim: 'none'},
      executionProtocol: {
        candidateRunId: 'semantic-manifold-g1-candidate-run-v29',
        resultPath: 'output/qualification/semantic-manifold-g1-candidate-run-v29/result.json',
        priorResultsMayBeImported: false,
        plannedWorkUnits: 4740,
      },
    })
