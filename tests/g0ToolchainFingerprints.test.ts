...
const fingerprintPath = resolve(
  repositoryRoot,
  'docs/qualification/g0-toolchain-fingerprints-v23.json',
)
...
describe('G0 toolchain fingerprints', () => {
  it('uses the frozen G0.2 schema id', () => {
    ...
    expect(doc.fingerprintId).toBe('g0-toolchain-fingerprints-v23')
    expect(doc).toMatchObject({
      previousFingerprintId: 'g0-toolchain-fingerprints-v22',
      previousFingerprintSha256: '51315ca494ad19f26fbff68c214014797accb27c0d8e08dcea37e3558222ae02',
    })
  })
...
describe('G0/G1 re-freeze generator', () => {
  ...
  it('prepares deterministic new versions without creating any artifact or result', () => {
    ...
    expect(first.plan.planId).toBe('semantic-manifold-g1-plan-v40')
  ...
  it('refuses a zero-counter freeze when the new candidate already has result bookkeeping', () => {
    const root = fixture()
    const prepared = prepare(root)
    const resultPath = resolve(root, 'output/qualification/semantic-manifold-g1-candidate-run-v40/result.json')
  ...
  it('historical v24 executors retain run-index admission without executing or creating evidence', () => {
    ...
    for (const path of REFRESH_EXECUTORS.slice(-2)) {
      ...
      const historical = readFileSync(resolve(root, path), 'utf8')
        .replaceAll('semantic-manifold-g1-plan-v40', 'semantic-manifold-g1-plan-v24')
        .replaceAll('semantic-manifold-g1-candidate-run-v40', 'semantic-manifold-g1-candidate-run-v24')
        .replace('G1 v40 clean-run', 'G1 v24 clean-run')
