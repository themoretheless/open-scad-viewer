...
describe('native authority survives scene publication',()=>{
 it('shows a native surface with independent display detail and preserves f64 source',async ()=>{
  const low=await buildTextNurbsScene(document,'preview'),full=await buildTextNurbsScene(document,'full')
 ...
 it('rejects a face reference after source geometry changes',async ()=>{
  const before=(await buildTextNurbsScene(document)).meshes[0]
  const changed=structuredClone(document);changed.nodes[0].control_points[1][1][2]=2
  const after=(await buildTextNurbsScene(changed)).meshes[0]
 ...
 it('preserves explicit trimming in the native revision without including sampling density',async ()=>{
  ...
  const first=(await buildTextNurbsScene(make(4,0.8))).meshes[0],fine=(await buildTextNurbsScene(make(12,0.8))).meshes[0],cut=(await buildTextNurbsScene(make(4,0.5))).meshes[0]
 ...
 it('rejects mutated snapshots at the worker boundary',async ()=>{
  const result=await buildTextNurbsScene(document)
