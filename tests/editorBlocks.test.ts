...
import { editorBlocks, indentSelection, guideFitsIndent } from '../src/services/editorBlocks'
...
it('inserts to a tab stop and handles tab-indented outdent',()=>{
 expect(indentSelection('a',1,1).text).toBe(' ')
 expect(indentSelection('',0,0).text).toBe('  ')
...
it('stops the inner guide before a closing bracket while retaining the outer guide',()=>{
 expect(guideFitsIndent('    )',4)).toBe(false)
 expect(guideFitsIndent('    )',0)).toBe(true)
 expect(guideFitsIndent('        sphere(2)',4)).toBe(true)
 expect(guideFitsIndent('\t)',4)).toBe(false)
 expect(guideFitsIndent('',4)).toBe(true)
})
